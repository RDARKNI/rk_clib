...
///
/// This header provides a lightweight, bitset representation and
/// helper functions for bit manipulation. Bit numbering is **0-based**:
/// bit index 0 refers to the **least-significant bit (LSB)** of `bs[0]`.
///
/// ## Storage model
/// A bitset is stored as an array of `bitset_word` (by default `unsigned long
/// long`):
...
/// *padding bits* with indices `>= nbits`. Many operations in this header
/// (e.g. `bitset_any`, `bitset_count_ones`, `bitset_equals`) operate on whole
/// words, so callers, if manipulating the bitset outside of the functions
/// defined here, must maintain the invariant.
///
/// @par Padding invariant
/// **All padding bits (indices `>= nbits`) are zero.**
///
/// Functions that write whole words and are documented to preserve correctness
/// will clear padding bits on return (e.g. `bitset_set_all`, shifts,
/// `bitset_not`, `bitset_sub`). If you introduce data by other means (e.g.
/// uninitialized storage, raw `memcpy`, manual word writes), call
/// `bitset_clear_padding()`.
///
/// ## Relationship to C23 stdbit.h conventions
/// Single-bit and range operations use 0-based bit indices.
/// The query functions `bitset_first_*` follow the C23 `<stdbit.h>` / common
/// builtin convention of returning a **1-based position**, with **0** as the
...
/// @brief Declares a fixed-size bitset object type with storage sufficient for
/// `nbits` bits while preserving the size in the type system.
...
/// ```
/// @note This macro expects `nbits` to be an integer
///       constant expression when used for object declarations.
#define bitset(nbits)                                                          \
  typeof(bitset_word[(                                                         \
      static_assert_expr((nbits) > 0, "Bitset must have at least one bit")     \
      + bitset_words(nbits))])

/// @brief Returns the number of storage words required for a `nbits`-bit
/// bitset.
/// @param nbits Logical size in bits
/// @return Word count, rounded up.
#define bitset_words(nbits)                                                    \
  (((nbits) + bitsof(bitset_word) - 1) / bitsof(bitset_word))
...
/// @brief Returns the storage word index containing bit `idx`.
/// @param idx Bit index (0-based)
/// @return Word index (0-based).
...
/// @note This copies whole words. If you rely on the padding invariant, ensure
///       `src` has cleared padding.
static_fun bitset bitset_copy(bitset restrict dst, size_t nbits,
                              cbitset restrict src) {
...
/// @return `bs` (for chaining).
/// @pre `idx < nbits`.
static_fun bitset bitset_write(bitset bs, size_t nbits, size_t idx,
                               bool value) {
  rk_assert_bitset_in_bounds(idx, nbits), (void)nbits;
...
static_fun rk_forceinline bitset RK__internal_bitset_range_op(bitset, size_t,
                                                              size_t, size_t,
                                                              int);
...
static_fun bitset bitset_clear_range(bitset bs, size_t nbits, size_t start,
                                     size_t end) {
...
/// @return `bs` (for chaining).
/// @pre `start <= end && end <= nbits`.
static_fun bitset bitset_set_range(bitset bs, size_t nbits, size_t start,
                                   size_t end) {
...
/// @return `bs` (for chaining).
/// @pre `start <= end && end <= nbits`.
static_fun bitset bitset_write_range(bitset bs, size_t nbits, size_t start,
                                     size_t end, bool value) {
...
/// @return `bs` (for chaining).
/// @pre `start <= end && end <= nbits`.
static_fun bitset bitset_flip_range(bitset bs, size_t nbits, size_t start,
                                    size_t end) {
  return RK__internal_bitset_range_op(bs, nbits, start, end, -1);
}

/// @brief Clears any padding bits (indices `>= nbits`) in the last storage
/// word.
/// @param bs,nbits Bitset and its logical size
/// @return `bs` (for chaining).
/// @note Call this if `bs` may contain nonzero padding bits (e.g. after
///       uninitialized allocation or raw word operations).
...
/// @note This matches the “1-based with 0 sentinel” convention used by C23
///       `<stdbit.h>` query functions and several compiler builtins.
...
static_fun size_t bitset_first_leading_zero(cbitset bs, size_t nbits) {
  ...
  size_t rest = nbits % bitset_word_bits, w = bitset_words(nbits) - 1, pos;
  if (rest) {
    pos = stdc_first_leading_zero(
        (bs[w] << (bitset_word_bits - rest))
...
static_fun bool bitset_none(cbitset bs, size_t nbits) {
  return !bitset_any(bs, nbits);
}
...
/// @brief Bitwise subtraction (in-place): clears bits present in `src`
///        (`dst &= ~src`).
/// @param dst Destination bitset (modified)
/// @param nbits Logical size of both bitsets
/// @param src Source bitset
...
static_fun bitset bitset_shift_left_into(bitset dst, size_t nbits, cbitset src,
                                         size_t sh) {
...
/// @note If `sh >= nbits`, the result is all zeros.
/// @note `dst` may alias `src`.
static_fun bitset bitset_shift_right_into(bitset dst, size_t nbits, cbitset src,
                                          size_t sh) {
  if (sh >= nbits) { return bitset_clear_all(dst, nbits), dst; }
  if (!sh) { return (cbitset)dst != src ? bitset_copy(dst, nbits, src) : dst; }
  size_t ws = sh / bitsof(*dst), bs = sh % bitsof(*dst);
  for (size_t i = 0, words = bitset_words(nbits); i < words; ++i) {
    ...
    bitset_word v = (bitset_word)src[si] >> bs;
    if (bs && (si + 1) < words) {
      v |= (bitset_word)src[si + 1] << (bitsof(*dst) - bs);
    }
    dst[i] = v;
  }
...
/// @param cur Previously visited bit index, or `(size_t)-1` to start from the
///            beginning
/// @return The index (0-based) of the first set bit with index `> cur`,
///         or `(size_t)-1` if no such bit exists.
/// @note This is an efficient iteration primitive for walking only the set
/// bits. It scans by storage word and uses trailing-one queries, so it is
/// typically much faster than testing every bit individually.
///
/// Usage:
/// ```c
/// bitset(128) bs = {0};
...
/// size_t i = (size_t)-1;
/// for (;(i = bitset_next_set(bs, 128, i)) != (size_t)-1;) {
///    printf("set bit: %zu\n", i);
/// }
/// // prints: 1, 5, 64
/// ```
static_fun size_t bitset_next_set(cbitset bs, size_t nbits, size_t cur) {
  enum { W = bitsof(*bs) };
  if (++cur >= nbits) { return (size_t)-1; }
...
/// @param cur Previously visited bit index, or `(size_t)-1` to start from the
///            beginning.
/// @return The index (0-based) of the first zero bit with index `> cur`,
///         or `(size_t)-1` if no such bit exists.
static_fun size_t bitset_next_clear(cbitset bs, size_t nbits, size_t cur) {
  enum { W = bitsof(*bs) };
  if (++cur >= nbits) { return (size_t)-1; }
  size_t      res, rest = nbits % W, w = bitset_word_index(cur);
  bitset_word valid = (rest && w == bitset_words(nbits) - 1)
                        ? (((bitset_word)1 << rest) - 1)
                        : ~(bitset_word)0;
  if ((res = stdc_first_trailing_one(~bs[w] & valid
                                     & (~(bitset_word)0 << (cur % W))))) {
    return w * W + res - 1;
  }
  for (size_t words = bitset_words(nbits); ++w < words;) {
    valid = (rest && w == words - 1) ? (((bitset_word)1 << rest) - 1)
                                     : ~(bitset_word)0;
    if ((res = stdc_first_trailing_one(~bs[w] & valid))) {
      return w * W + res - 1;
    }
...
/// @brief Finds the previous set bit before `cur` when scanning from MSB to
///        LSB.
/// @param bs,nbits Bitset and its logical size.
/// @param cur Current bit index, or `nbits` to start from the end.
/// @return The index (0-based) of the last set bit with index `< cur`,
///         or `(size_t)-1` if no such bit exists.
static_fun size_t bitset_prev_set(cbitset bs, size_t nbits rk_unused,
                                  size_t cur) {
  enum { W = bitsof(*bs) };
  if (!cur) { return (size_t)-1; }
  --cur;
  size_t      res, w = bitset_word_index(cur);
  bitset_word mask = ~(bitset_word)0 >> (W - 1 - (cur % W));
  if ((res = stdc_first_leading_one(bs[w] & mask))) {
    return w * W + (W - res);
  }
  for (; w--;) {
...
/// @brief Finds the previous zero bit before `cur` when scanning from MSB to
///        LSB.
/// @param bs,nbits Bitset and its logical size.
/// @param cur Current bit index, or `nbits` to start from the end.
/// @return The index (0-based) of the last zero bit with index `< cur`,
///         or `(size_t)-1` if no such bit exists.
static_fun size_t bitset_prev_clear(cbitset bs, size_t nbits, size_t cur) {
  enum { W = bitsof(*bs) };
  if (!cur) { return (size_t)-1; }
  --cur;
  size_t      res, rest = nbits % W, w = bitset_word_index(cur);
  bitset_word valid = (rest && w == bitset_words(nbits) - 1)
                        ...
                        : ~(bitset_word)0;
  bitset_word mask  = ~(bitset_word)0 >> (W - 1 - (cur % W));
  if ((res = stdc_first_leading_one(~bs[w] & valid & mask))) {
    return w * W + (W - res);
  }
  for (; w--;) {
    if ((res = stdc_first_leading_one(~bs[w]))) { return w * W + (W - res); }
...
/// @brief Finds the first set bit when scanning from LSB to MSB.
/// @param bs,nbits Bitset and its logical size.
/// @return The index (0-based) of the first set bit, or `(size_t)-1` if none.
static_fun size_t bitset_first_set(cbitset bs, size_t nbits) {
  return bitset_next_set(bs, nbits, (size_t)-1);
}

/// @brief Finds the first zero bit when scanning from LSB to MSB.
/// @param bs,nbits Bitset and its logical size.
/// @return The index (0-based) of the first zero bit, or `(size_t)-1` if none.
static_fun size_t bitset_first_clear(cbitset bs, size_t nbits) {
  return bitset_next_clear(bs, nbits, (size_t)-1);
}

/// @brief Finds the last set bit when scanning from MSB to LSB.
/// @param bs,nbits Bitset and its logical size.
/// @return The index (0-based) of the last set bit, or `(size_t)-1` if none.
static_fun size_t bitset_last_set(cbitset bs, size_t nbits) {
  return bitset_prev_set(bs, nbits, nbits);
}

/// @brief Finds the last zero bit when scanning from MSB to LSB.
/// @param bs,nbits Bitset and its logical size.
/// @return The index (0-based) of the last zero bit, or `(size_t)-1` if none.
static_fun size_t bitset_last_clear(cbitset bs, size_t nbits) {
  return bitset_prev_clear(bs, nbits, nbits);
}
...
/// @param nbits Logical size of the bitset
/// @return `dst` (for convenience).
/// The output is `nbits` characters of `'0'`/`'1'`, plus a trailing `'\0'`.
static_fun char* bitset_tostr(char* restrict dst, cbitset restrict src,
                              size_t nbits) {
  for (size_t w = 0; w < nbits; ++w) {
    dst[w] = '0' + bitset_test(src, nbits, nbits - 1 - w);
  }
  dst[nbits] = '\0';
  return dst;
}
...
/// @param dst Destination bitset storage for a **len-bit** bitset
///            (at least `bitset_words(len)` words).
/// @param src Source string of at least `len` characters, consisting only of
///            `'0'` and `'1'`. The first character corresponds to the MSB.
/// @param len Number of bits to parse; also the logical size of the resulting
/// bitset.
/// @return `dst` (for chaining).
/// @note This function treats `dst` as a `len`-bit bitset for this call.
///       It writes all bits `[0, len)` and clears padding bits on return.
static_fun bitset bitset_fromstr(bitset dst, const char* restrict src,
                                 size_t len) {
  for (size_t w = 0; w < len; ++w) {
    rk_assert((src[w] == '0' || src[w] == '1')
              && "Invalid character in bitset string");
...
static_fun rk_forceinline bitset RK__internal_bitset_range_op(
    bitset bs, size_t nbits, size_t start, size_t end, int op) {
  ...
  bitset_word sm = (~(bitset_word)0 << sb),
              em = (~(bitset_word)0 >> (bitset_word_bits - (eb + 1)));
  if (sw == ew) {
    switch (op) {
    case -1: bs[sw] ^= (sm & em); break;
    case 0 : bs[sw] &= ~(sm & em); break;
    case 1 : bs[sw] |= (sm & em); break;
    default: unreachable();
    }
    return bs;
  ...
  if (sb) {
    switch (op) {
    case -1: bs[sw] ^= sm; break;
    case 0 : bs[sw] &= ~sm; break;
    case 1 : bs[sw] |= sm; break;
    default: unreachable();
    }
    ++sw;
  ...
  case -1:
    ...
    bs[ew] ^= em;
    break;
  case 0 : memset(&bs[sw], 0, sizeof_n(*bs, (ew - sw))), bs[ew] &= ~em; break;
  case 1 : memset(&bs[sw], 0xFF, sizeof_n(*bs, (ew - sw))), bs[ew] |= em; break;
  default: unreachable();
  }
  return bs;
}
