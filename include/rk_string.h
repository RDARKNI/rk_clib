...
/// @brief A small custom header-only String library for working with
/// dynamically allocated Strings and String views.
/// This library provides functionality to create, manipulate, and manage
/// custom String Structs (`Str`), which are dynamically allocated with
/// automatic resizing based on the length of the String. It also provides a
/// simple immutable String view type, `Strv`.
///
/// @section Customisation Points
/// Customisation points are the same as for all other rk_clib headers, most
/// notably the `RK_ALLOCMODE` macro which, if not set to `RK_ALLOCMODE_FULL`,
/// turns off custom allocators, removing the respective data members from the
/// structs and parameters from the functions, while discarding the allocator
/// parameters in the user-facing macros, removing any overhead.
///
/// @section Data Types
/// This Header Provides two Data Types, each one having a subset of the
/// abilities of the latter
/// - `Str`: A mutable String with dynamic memory allocation.
/// - `Strv`: A non-owning view over a String.
///
/// @section Function Parameters and Implicit Conversions Internally, most
/// functions are defined as taking a `Strv` parameter; the macros in this
/// header allow for passing of any Stringlike object, expanding them
/// automatically to the required argument types.
/// @see rk_alloc.h
/// @see rk_defs.h
/// @{
...
/// @note `(Strv){NULL, 0}` is a valid `Strv` and will be treated accordingly by
...
/// @brief Represents a mutable, null-terminated string buffer.
/// The string is represented by a null-terminated dynamic array of characters
/// along with its current length and allocated capacity and an optional
/// Allocator member. Every `Str` object can be safely cast into a Strv,
/// either via pointer cast or by taking its `v` member.
/// A valid `Str` satisfies:
/// - `str` points to a buffer of at least `cap` bytes or is NULL if cap == 0
/// - if `str` is not null, it is null-terminated with `str[len] == '\0'`
/// - `len <= cap`
/// - Functions generally preserve null-termination unless explicitly stated.
///   The only exceptions in this API are `str_release()` and the `_raw()`
///   functions.
typedef struct Str {
  union {
...
/// - `char*`
/// - `const char*`
/// These are implicitly converted to `Strv` via internal macros.
// --------------------------------
...
#define str_front(strlike)                                                     \
  (*RK__qcharptr(strlike, RK__str_front_ptr(strv_from(strlike))))
...
/// @note behaviour undefined for empty strings
#define str_back(strlike)                                                      \
  (*RK__qcharptr(strlike, RK__str_back_ptr(strv_from(strlike))))

// ----------------------------------------
// Section: String Lifetime/Ownership
// ----------------------------------------

/// @brief `Str str_init(size_t init_cap,  Allocator alloc = alloc_ctx)` -
/// Constructs a Str with a given initial capacity.
/// @param init_cap size_t The initial capacity of the string (in elements)
/// @param alloc    Allocator Optional parameter - The allocator; defaults to
///                 `alloc_ctx` if not provided
/// @return A `Str` object with the given capacity
#define str_init(init_cap, ...)                                                \
  rk_overload(RK__str_init, init_cap, ##__VA_ARGS__)

/// @brief `Str str_from(Strlike strlike, Allocator alloc = alloc_ctx)` -
/// Constructs a Str from a Stringlike object, copying the data.
/// @param strlike     A Stringlike, by value
/// @param alloc       Allocator Optional parameter - The allocator
/// backing the new Str. If not provided, this defaults to the Allocator of the
/// cloned Strlike if it is a Str object, or alloc_ctx otherwise.
...
/// @param alloc   Allocator Optional parameter - The allocator; defaults
/// to `alloc_ctx` if not provided
/// @return A `Str` object initialised with the literal's contents
#define str_from_literal(strlit, ...)                                          \
  rk_overload(RK__str_fromlit, strlit, ##__VA_ARGS__)

/// @brief Frees the underlying memory of self.
static_fun void str_release(Str* restrict self) {
  if (self->str) { alloc_delete(self->str, self->cap, self->alloc); }
  self->len = self->cap = 0, self->str = (char*)0;
}

/// @brief `Str str_join_strv_n(Strv* svs, size_t count, Strv sep, Allocator
/// alloc = alloc_ctx)` - Constructs a Str by concatenating `count` elements
/// from `svs`, inserting `sep` between each element.
...
/// @brief Ensures at least `new_cap` bytes of capacity are allocated for
/// `self`, reallocating, if necessary.
static_fun Str* str_reserve(Str* restrict self, size_t new_cap);

/// @brief Resizes the length of `self` to `new_len`, reallocating the memory if
/// necessary and null-terminating it.
static_fun Str* str_resize(Str* restrict self, size_t new_len);

/// @brief Resizes a Str's capacity to the next power of two larger than its
/// length, null-terminating it.
...
// ----------------------------------------
/// @brief Assigns a new value to a `Str` from a Stringlike, overriding its
/// contents. The Str is resized if necessary to fit the new data.
...
/// @brief Terminates a `Str` with `suffix` if it does not already end with it
/// and then null-terminates it if it isn't.
...
/// @note Appends int as a character of its value, doing `str_push(s, 1)` is
/// likely a bug.
...
/// @brief Generic Convenience Macro for appending Strlikes or characters to a
/// string, automatically converting `strlike` to Strv and null-terminating
/// `self`. May reallocate `self`.
/// @param self    The String to append to
/// @param strlike The Stringlike to append
/// @return The updated String.
/// @note Appends int as a character of its value, `str_cat(s, 1)` is
/// likely wrong.
/// @attention Appending a substring of `self` (i.e. overlapping memory)
/// results in undefined behavior. Use `str_cat_mayalias()` in that case.
...
/// @brief Like `str_cat()`, specialised on string literals.
/// Avoids a runtime `strlen()` call via compile-time `sizeof()` and ensures a
/// compile-time error if `strlit` is not a string literal.
#define str_cat_literal(self, strlit)                                          \
  str_cat_strv(self, (Strv)strv_from_literal(strlit))

/// @brief Appends formatted output (printf-style) to the string.
/// The format string and arguments follow the rules of the `printf` family.
/// @param self The string to append to
...
/// @return `self`, for chaining
/// @attention Appending a substring of self to self via this is undefined
#define str_insert_at(self, idx, strlike) RK__str_insert_at(self, idx, strlike)

/// @brief Like `str_insert_at()` but checks for pointer aliasing between self
/// and strlike.
#define str_insert_at_mayalias(self, idx, strlike)                             \
  RK__str_insert_at_mayalias(self, idx, strlike)

/// @brief Pops the last character off the Str, decreasing its length and
/// null-terminating it.
...
/// @brief Pops the last n character off the Str, decreasing its length and
/// null-terminating it.
/// @note no-op for size 0 strings. Popping more characters than the length of
/// the Str is asserted in debug builds.
static_fun void str_pop_n(Str* restrict self, size_t n) {
  if (!n) { return; }
  rk_assert(self->cap && n <= self->len
            && "Attempted to pop more than Str length");
...
/// @brief Convenience Macro to erase all elements in `self` that satisfy a
/// predicate.
...
#define str_erase_if(self, it, pred)                                           \
  do {                                                                         \
    ...
    for (const char *RK__IT = RK__READ, *const RK__END                         \
                                        = RK__READ + RK__STR->len;             \
         RK__IT < RK__END; ++RK__IT) {                                         \
      const char* const it = RK__IT;                                           \
      if (!(pred)) { *RK__WRITE++ = *RK__IT; }                                 \
    }                                                                          \
    RK__STR->len               = RK__WRITE - RK__STR->str;                     \
    RK__STR->str[RK__STR->len] = '\0';                                         \
  } while (0)

/// @brief Clears the contents of `self`, setting its length to zero and
/// null-terminating it, if it owns an allocation.
...
/// @brief Replaces the character at position `pos` in `self` with `c`.
/// Does not modify the `len` field, even if `c == '\0'`. This allows
/// temporarily inserting a null terminator within the string to treat a
/// substring as a C-string, and later restoring the original character without
...
// ----------------------------------------
/// @brief Constructs a Strv from a string literal; useful for compile-time
/// initialisation. Verifies that `strlit` is a string literal.
...
/// @brief Creates a Strv from a substring of a Stringlike.
/// If `start > str_len(strlike)`, an empty view is returned.
/// if `start + count > str_len(strlike)` is larger than length, the String from
...
#define strv_slice(strlike, start, end)                                        \
  strv_slice_strv(strv_from(strlike), start, end)

/// @brief Creates a Strv from a substring of a char array (or pointer) at
/// compile time.
...
/// @brief Tokenises the string without modifying the underlying data.
/// The function consumes the view pointed to by `strvptr`, returning
/// successive tokens separated by `delims`.
/// The original string data is never modified. Instead, `*strvptr` is
/// advanced to point to the remaining unconsumed portion of the view.
/// When the final token has been returned, `strvptr->len` is set to
/// `STR_SPLIT_END`. Callers must stop iterating
/// once this sentinel value is observed.
///
/// @param strvptr Pointer to the view being consumed The view is modified in
/// place.
/// @param delims The delimiters used for tokenisation May be a character,
/// integer (character literal), or any Stringlike type. Each occurrence of a
/// delimiter separates tokens.
/// @return A `Strv` representing the next token. The returned view refers
/// directly into the original string data.
///
/// Usage:
/// ```c
...
/// @brief Tokenises the Stringlike without modifying the underlying data,
/// allocating an array of Strv Objects.
/// @param strlike   The Stringlike to tokenise (by value)
/// @param delims    The delimiters of the tokenisation, a Stringlike by value
/// @param out_count size_t* an out-parameter in which the count of tokens will
///                  be stored.
/// @param alloc     Optional; The Allocator that allocates the array
///                  of Strv objects; defaults to `alloc_ctx` (or
///                  malloc_allocator, if `RK_ALLOCMODE` ==
///                  `RK_ALLOCMODE_MALLOC_ONLY`)
/// @return A Strv* to the array of tokens.
#define str_split_alloc(strlike, delims, out_count, ...)                       \
  rk_overload(RK__str_split_alloc, strv_from(strlike), strv_from(delims),      \
              out_count, ##__VA_ARGS__)

/// @brief Trims a Stringlike into a string by adjusting its starting
/// position to the first non-space character it finds.
...
///         respectively.
/// @note If the strings are equal up to the length of the shorter one,
/// the result is the difference in their lengths.
#define str_compare(strlike1, strlike2)                                        \
  str_compare_strv(strv_from(strlike1), strv_from(strlike2))

/// @brief Returns whether two Stringlikes are lexicographically equal.
#define str_equals(strlike1, strlike2)                                         \
  str_equals_strv(strv_from(strlike1), strv_from(strlike2))
...
/// @return A `(const) char*` to the first occurrence or a nullpointer if the
/// substring is not found.
#define str_find(strlike, subs)                                                \
  RK__qcharptr(strlike, RK__str_find(strlike, subs))

/// @brief Finds the last occurrence of `subs` in `strlike`.
/// @param strlike  A Stringlike, the haystack
/// @param subs     A Stringlike, the needle
/// @return A `(const) char*` to the last occurrence or a nullpointer if the
/// substring is not found.
#define str_findr(strlike, subs)                                               \
  RK__qcharptr(strlike, RK__str_findr(strlike, subs))
...
#define RK__str_insert_at(self, idx, strlike)                                  \
  RK__IsCharLitLike(                                                           \
      strlike, str_insert_at_char(self, idx, RK__GetCharLitLike(strlike)),     \
...
#define RK__str_split(strvptr, delims)                                         \
  RK__IsCharLitLike(delims,                                                    \
                    str_split_char(strvptr, RK__GetCharLitLike(delims)),       \
                    str_split_strv(strvptr, RK__strv_from_fallback(delims)))

#define RK__str_terminate(self, suffix)                                        \
  RK__IsCharLitLike(suffix,                                                    \
                    str_terminate_char(self, RK__GetCharLitLike(suffix)),      \
...
#define RK__str_find(strlike, subs)                                            \
  RK__IsCharLitLike(                                                           \
      subs, str_find_char(strv_from(strlike), RK__GetCharLitLike(subs)),       \
      str_find_strv(strv_from(strlike), RK__strv_from_fallback(subs)))

#define RK__str_findr(strlike, subs)                                           \
  RK__IsCharLitLike(                                                           \
      subs, str_findr_char(strv_from(strlike), RK__GetCharLitLike(subs)),      \
      str_findr_strv(strv_from(strlike), RK__strv_from_fallback(subs)))

#define RK__str_contains(strlike, subs)                                        \
  RK__IsCharLitLike(                                                           \
      subs, str_contains_char(strv_from(strlike), RK__GetCharLitLike(subs)),   \
      str_contains_strv(strv_from(strlike), RK__strv_from_fallback(subs)))

#define RK__str_starts_with(strlike, prefix)                                   \
  RK__IsCharLitLike(                                                           \
      prefix,                                                                  \
      str_starts_with_char(strv_from(strlike), RK__GetCharLitLike(prefix)),    \
      str_starts_with_strv(strv_from(strlike),                                 \
                           RK__strv_from_fallback(prefix)))

#define RK__str_ends_with(strlike, prefix)                                     \
  RK__IsCharLitLike(                                                           \
      prefix,                                                                  \
      str_ends_with_char(strv_from(strlike), RK__GetCharLitLike(prefix)),      \
      str_ends_with_strv(strv_from(strlike), RK__strv_from_fallback(prefix)))

static_fun Strv strv_from_cstr(const char* s) {
  return (Strv){.str = s, .len = s ? strlen(s) : 0};
}

static_fun Strv strv_from_strv(Strv str) { return str; }
#define RK__str_fromlit2(a, alloc)                                             \
  str_from_strv((Strv)strv_from_literal(a) RK_IFALLOC(, alloc))
#define RK__str_fromlit1(a) RK__str_fromlit2(a, alloc_ctx)
...
static_fun bool str_starts_with_char(Strv sv, char c) {
  return sv.len && sv.str[0] == c;
}
static_fun bool str_starts_with_strv(Strv sv, Strv pref) {
  return pref.len <= sv.len && !rk_memcmp(sv.str, pref.str, pref.len);
}
static_fun bool str_ends_with_char(Strv sv, char c) {
  return sv.len && sv.str[sv.len - 1] == c;
}

static_fun bool str_ends_with_strv(Strv sv, Strv suf) {
  return suf.len <= sv.len
      && !rk_memcmp(sv.str + sv.len - suf.len, suf.str, suf.len);
}

static_fun const char* str_find_char(Strv sv, char c) {
  return sv.str ? (const char*)memchr(sv.str, (unsigned char)c, sv.len)
                : sv.str;
}

extern_fun const char* str_find_strv(Strv hs, Strv ne) {
  typedef unsigned char uchar;
  if (ne.len == 0) { return hs.str; }
  if (ne.len > hs.len) { return (char*)0; }
  if (ne.len <= 3) {
    ...
    const uchar *hstr = (uchar*)hs.str, *nstr = (uchar*)ne.str;
    size_t       i;
    uchar        skip[256];
    for (i = 0; i < countof(skip); ++i) { skip[i] = ne.len; }
    for (i = 0; i < ne.len - 1; ++i) {
      skip[nstr[i]] = rk_MIN(ne.len - i - 1, (size_t)255);
    }
    for (size_t i = 0; i <= hs.len - ne.len;) {
      uchar last = hstr[i + ne.len - 1];
      if (last == nstr[ne.len - 1] && !memcmp(hstr + i, nstr, ne.len - 1)) {
        return hs.str + i;
      }
      i += skip[last];
    }
...
extern_fun const char* str_findr_char(Strv sv, char c) {
  while (sv.len--) {
    if (sv.str[sv.len] == c) { return sv.str + sv.len; }
  }
  return (const char*)0;
}
extern_fun const char* str_findr_strv(Strv hs, Strv ne) {
  if (ne.len == 0) { return hs.str; }
  if (ne.len > hs.len) { return (char*)0; }
...
extern_fun Str RK__str_init(size_t cap RK_IFALLOC(, Allocator alloc)) {
  rk_assert_allocator_valid(alloc);
  cap       = stdc_bit_ceil(cap); // 1 if cap == 0 (-> space for terminator)
  char* str = alloc_new(char, cap, alloc);
  str[0]    = '\0';
  return (Str){.str = str, .len = 0, .cap = cap RK_IFALLOC(, .alloc = alloc)};
}

#define RK__str_init2(cap, alloc) RK__str_init(cap RK_IFALLOC(, alloc))
#define RK__str_init1(cap)        RK__str_init2(cap, alloc_ctx)

static_fun void RK__str_change_cap(Str* restrict self, size_t new_cap) {
  rk_set_alloc_fallback(self->alloc);
  self->str = alloc_renew(self->str, self->cap, new_cap, self->alloc);
  self->cap = new_cap;
}
...
extern_fun Str str_from_strv(Strv sv RK_IFALLOC(, Allocator alloc)) {
  rk_assert_allocator_valid(alloc);
  ...
  str[sv.len] = '\0';
  return (Str){
      .str = str, .len = sv.len, .cap = cap RK_IFALLOC(, .alloc = alloc)};
}

#define RK__str_from2(_S, alloc)                                               \
  str_from_strv(strv_from(_S) RK_IFALLOC(, alloc))
#define RK__str_from1(_S)                                                      \
  RK__str_from2(                                                               \
      _S, _Generic(_S, Str: RK__contrav(Str, _S).alloc, default: alloc_ctx))

extern_fun Str RK__str_join_strv_n(Strv* svs, size_t count,
  ...
  res.str    = alloc_new(char, res.cap, alloc);
  size_t pos = 0;
  for (size_t i = 0; i < count; ++i) {
    if (svs[i].len) {
      memcpy(res.str + pos, svs[i].str, svs[i].len), pos += svs[i].len;
    }
    if (i + 1 < count && sep.len) {
      memcpy(res.str + pos, sep.str, sep.len), pos += sep.len;
    }
  }
  res.str[res.len] = '\0';
  return res;
...
#define RK__str_join_strv_n4(svs, count, sep, alloc)                           \
  RK__str_join_strv_n(svs, count, sep, alloc)
#define RK__str_join_strv_n3(svs, count, sep)                                  \
  RK__str_join_strv_n4(svs, count, sep, alloc_ctx)

extern_fun Str* str_assign_strv(Str* restrict self, Strv sv) {
...
extern_fun Str* str_resize(Str* restrict self, size_t new_len) {
...
extern_fun Str* str_cat_strv(Str* restrict self, Strv sv) {
...
extern_fun Str* str_cat_strv_mayalias(Str* restrict self, Strv sv) {
  if (!sv.len) { return self; }
  size_t new_len = sv.len + self->len;
...
extern_fun Str* str_insert_at_strv(Str* restrict self, size_t at, Strv sv) {
...
extern_fun Str* str_insert_at_strv_mayalias(Str* restrict self, size_t idx,
                                            Strv sv) {
  ...
  if (alias) {
    rk_set_alloc_fallback(self->alloc);
    from = alloc_new(char, sv.len, self->alloc);
  ...
  if (alias) { alloc_delete(from, sv.len, self->alloc); }
  self->len = new_len;
  return self;
}

extern_fun Str* str_insert_at_char(Str* restrict self, size_t at, char chr) {
...
static_fun Str* str_null_terminate_unchecked(Str* restrict self) {
  rk_assert(self->cap > self->len
            && "Attempted to push beyond the capacity of the Str");
...
extern_fun Str* str_erase_at_n(Str* restrict self, size_t idx, size_t count) {
  rk_assert(idx <= self->len && count <= self->len - idx
            && "Erase out of bounds");
  if (count) {
    memmove(self->str + idx, self->str + idx + count,
            self->len - idx - count + 1);
    self->len -= count;
  }
  return self;
}
...
static_fun Strv str_trimmed_left_strv(Strv sv) {
  ...
  return (Strv){.str = i ? sv.str + i : rk_null, .len = sv.len - i};
}
...
extern_fun Strv*(
    str_split_alloc)(Strv str, Strv dels,
  ...
  Strv*  data = alloc_new(Strv, count, alloc);
...
#define RK__str_split_alloc3(str, delims, count)                               \
  RK__str_split_alloc4(str, delims, count, alloc_ctx)
...
/*Note to readers:
alloc_ macros ignore the last macro parameter if local allocators are disabled;
something like alloc_new(char, 5, self->alloc) will compile even if self->alloc
does not exist since self->alloc will be filtered out by the preprocessor.
*/
/// @endcond
