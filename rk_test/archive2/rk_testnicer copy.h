...
static struct {
  ...
  rk_fd             RK__OUT;
  union {
    char             resbufr_priv[2048];
    RKT_AssertResPkg resbufr;
  };
} RKT_glob;
...
public:
  ...
  size_t send(size_t res_val) {
    ...
    if (package_size < sizeof(RKT_glob.resbufr_priv)) {
      res = &RKT_glob.resbufr;
    } else {
    ...
    if (package_size >= sizeof(RKT_glob.resbufr_priv)) { free(res); }
    return res_val;
  }
...
#define RKT_SNDBUF0(RES)                                                       \
  RKT_glob.resbufr.res = (RES);                                                \
  RKT_glob.resbufr.len = 0;                                                    \
  return RKT_send_AssertRes(&RKT_glob.resbufr);
#define RKT_SNDBUF1(RES, e1, FMT1)                                             \
  RKT_glob.resbufr.res = (RES);                                                \
  RKT_glob.resbufr.len = sprintf(RKT_glob.resbufr.args, FMT1, e1) + 1;         \
  return RKT_send_AssertRes(&RKT_glob.resbufr);

// todo important arg gc e bug
#define RKT_SNDBUF2(RES, e1, FMT1, e2, FMT2)                                   \
  RKT_glob.resbufr.res = (RES);                                                \
  RKT_glob.resbufr.len                                                         \
      = sprintf(RKT_glob.resbufr.args, FMT1 "%c" FMT2, e1, '\0', e2) + 1;      \
  return RKT_send_AssertRes(&RKT_glob.resbufr);

#define RKT_SNDBUF3(RES, e1, FMT1, e2, FMT2, e3, FMT3)                         \
  RKT_glob.resbufr.res = (RES);                                                \
  RKT_glob.resbufr.len                                                         \
      = sprintf(RKT_glob.resbufr.args, FMT1 "%c" FMT2 "%c" FMT3, e1, '\0', e2, \
                '\0', e3)                                                      \
      + 1;                                                                     \
  return RKT_send_AssertRes(&RKT_glob.resbufr);
...
rkt_fun size_t RKTf_streq_n(const char* e1, const char* e2, size_t n) {
  RKT_AssertResPkg* res;
  if (!n) {
    res = &RKT_glob.resbufr, res->res = 0, res->len = 0;
...
rkt_fun size_t RKTf_strneq_n(const char* e1, const char* e2, size_t n) {
  RKT_AssertResPkg* res;
  if (!n) {
    res = &RKT_glob.resbufr, res->res = 1, res->len = 0;
    return RKT_send_AssertRes(res);
  }
  size_t dif          = RK_test_strdiff(e1, n, e2, n);
  size_t package_size = sizeof(*res) + n + n + 2;
