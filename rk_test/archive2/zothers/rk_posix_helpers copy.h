...
#define rk_set_nonblocking(...)                                                \
    (rk_set_nonblocking_n)((int[]){__VA_ARGS__},                               \
...
static inline int rk_set_nonblocking_n(int* fds, size_t nfds) {
    for (int i = 0, flags; i < nfds; ++i) {
        while ((flags = fcntl(fds[i], F_GETFL, 0)) == -1) {
            if (errno != EINTR) {
                return assert(!"fncl set nonblocking failed"), i;
            }
        }
...
/// @return 0 on success, index of fd that failed otherwise.
#define rk_close(...)                                                          \
    (rk_close_n)((rk_fd[]){__VA_ARGS__},                                       \
                 sizeof((rk_fd[]){__VA_ARGS__}) / sizeof(rk_fd))
...
static inline pid_t rk_fork_bipipe(int* r, int* w) {
    rk_Pipe ps[2];
    if (pipe(ps[0]._) < 0) { rk_abort("pipe"); };
    if (pipe(ps[1]._) < 0) { rk_close(ps[0].r, ps[0].w), rk_abort("pipe"); };
    pid_t pid = fork();
    if (pid == -1) {
        rk_close(ps[0].r, ps[0].w, ps[1].r, ps[1].w), rk_abort("fork");
    }
    int d = pid == 0;
...
static inline pid_t rk_fork_unipipe(int* fdptr, RK_UNIPIPE_DIR_t dir) {
    int pps[2];
    if (pipe(pps) < 0) { rk_abort("pipe"); };
    pid_t pid;
    if ((pid = fork()) < 0) { close(pps[0]), close(pps[1]), rk_abort("fork"); }
...
static inline pid_t rk_fork_unipipe_n(int* fdptr, RK_UNIPIPE_DIR_t dir,
    ...
    for (opened = 0; opened < npipes; ++opened) {
        if (pipe(pps + opened * 2) < 0) { goto err; };
    }
...
err:
    ...
    rk_abort("pipe"); // todo unreachable, type of error
    return 0;
}
