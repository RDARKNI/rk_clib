...
static struct {
  RKT_CustomAttrs attrs;
  const char* custom_paths[RK_OUTPUTFORMAT_COUNT]; // todo this system is a mess
  FILE* output_types[RK_OUTPUTFORMAT_COUNT]; // todo rename to default files?
...
rkt_fun bool RKT_shall_print(const RKT_TestEntry* e, RK_OutPutStreams s) {
  ...
  case RK_OUTPUTSTREAMS_META: return v != RK_VERBOSITY_NEVER;
  }
  assert(0);
}
...
#define RKT_SEND_HDR(ESTR, FUNENUM, SIG)                                       \
  RKT_send_hdr((RKT_AssertHdr){__LINE__, FUNENUM, SIG, {ESTR, lenof(ESTR)}})
  ...
  private:
    ...
    void grow_to(size_t ncap) {
      if (cap == mem_size) {
        ...
        if (!buf) { exit(1); } // todo out of memory
...
RKT_TYPELIST_NOFLOAT(RK_test_GENFUNS_nofloat)                
RKT_TYPELIST(RK_test_GENFUNS_alltypes)                       
...
#define RKT_fmtstub(_S, FMT, _E, _HDR)                                         \
  ...
  default:
    ...
    case RKTF_MEMEQ:
      s += sprintf(
          s, "Memory Regions %s and %s (%s bytes long) " RK_INRED("not equal"),
          args[0], args[1], args[2]);
      break;
    case RKTF_MEMNEQ:
      s += sprintf(
          s, "Memory Regions %s and %s (%s bytes long) " RK_INRED("equal"),
          args[0], args[1], args[2]);
      break;
    case RKTF_MEMZERO:
...
rkt_fun void RKT_print_test_tap(const RKT_TestEntry* e) {
  ...
  switch (e->res.ended) {
  case RKT_PASSED:
    fprintf(out, "PASS: Passed all %zu Assertions.", e->res.count);
    break;
  case RKT_FAILED:
    fprintf(out, "FAIL: %zu fails, %zu passed (total %zu)", e->res.fails,
            e->res.count - e->res.fails, e->res.count);
    break;
  case RKT_CRASHED:
    fprintf(out,
            "CRASH: %zu fails, %zu passed (total %zu), "
            "crashed unexpectedly with signal %d",
            e->res.fails, e->res.count - e->res.fails, e->res.count,
            RKT_interpret_code(e->res.term_code));
    break;
  case RKT_TIMEDOUT:
    fprintf(out,
            ...
            "%ldms",
            e->res.fails, e->res.count - e->res.fails, e->res.count,
            RKT_resolve_timeout(e));
    break;
  case RKT_TESTERROR: // todo windows
#ifndef _WIN32
    if (WIFSIGNALED(e->res.term_code)) {
      fprintf(out, "Error in Test Function; Crashed with signal %d",
              WTERMSIG(e->res.term_code));
    } else {
      fprintf(out, "Error in Test Function; Exited with code %d",
              WEXITSTATUS(e->res.term_code));
    }
#else
      ;
#endif
...
rkt_fun RKT_TestEndType RKT_run_test(RKT_TestEntry* e) {
  ...
  RKT_parent_loop(e, pid, pipes), ++RKT_glob.tests_run;
...
rkt_fun bool RKT_read_hdr(rk_fd fd, RKT_AssertHdr* restrict hdr) {
  switch (RKT_read_full_nb(fd, hdr, sizeof(RKT_AssertHdr))) {
...
rkt_fun bool RKT_read_res(rk_fd fd, RKT_AssertRes* restrict res) {
  switch (RKT_read_full_nb(fd, res, sizeof(RKT_AssertResPkg))) {
  ...
  case 1:
    if (!res->len) {
      ...
      if (RKT_read_full_nb(fd, res->args[0], res->len) < 1) {
        RKT_fatal("read OWO");
      }
      size_t i = 1;
...
rkt_fun RKT_TestEndType RKT_parent_loop(RKT_TestEntry* restrict e, rk_pid pid,
  ...
  for (int nfds = 3; nfds > 0;) {
    for (long poll_timeout = -1;;) {
      if (timeout_ms != -1) {
        time_elapsed = RKT_now_ms() - time_start;
        poll_timeout = RK_MAX(timeout_ms - time_elapsed, 0);
      }
      int pr = poll(fds, 3, (int)poll_timeout);
      if (pr > 0) { break; }
      if (pr == -1) {
    ...
    if (fds[RK_OUTPUTSTREAMS_META].revents & POLLIN) {
      if (!reading_res) {
        ...
        switch (RKT_read_hdr(fds[RK_OUTPUTSTREAMS_META].fd,
                             &r.results[r.count].hdr)) {
        case true : ++r.count, reading_res = 1; break;
        case false: fds[RK_OUTPUTSTREAMS_META].fd = -1, --nfds; break;
        }
      } else {
        switch (RKT_read_res(fds[RK_OUTPUTSTREAMS_META].fd,
                             &r.results[r.count - 1].res)) {
        case true : reading_res = 0; break;
        case false: fds[RK_OUTPUTSTREAMS_META].fd = -1, --nfds; break;
        }
      }
    }
    for (int i = 0; i < 2; ++i) {
      ...
      switch (RKT_read_stream(fds[i].fd, &r.capt[i], &caps[i])) {
      case true : break;
      case false: fds[i].fd = -1, --nfds; break;
      }
    }
  }
  close(pipes[0]), close(pipes[1]), close(pipes[2]);
    ...
    *lastres                     = (RKT_AssertRes){RKT_ZINIT};
    ...
    if (WIFSIGNALED(r.term_code)) {
      ...
      if (lasthdr->F == RKTF_CRASH) {
        ...
        if (lasthdr->code != -1 && sig != lasthdr->code) { lastres->res = 2; }
      } else {
      ...
      if (lasthdr->F == RKTF_EXIT) {
        ...
        if (lasthdr->code != -1 && stat != lasthdr->code) { lastres->res = 2; }
      } else {
  ...
  for (int nfds = 3; nfds > 0;) {
    ...
    if (handles[idx] == pipes[RK_OUTPUTSTREAMS_META]) {
      if (!reading_res) {
        ...
        ++r.count;
        switch (RKT_read_hdr(handles[idx], &r.results[r.count].hdr)) {
        case true : ++r.count, reading_res = 1; break;
        case false: handles[idx] = INVALID_HANDLE_VALUE; break;
        }
      } else {
        switch (RKT_read_res(handles[idx], &r.results[r.count - 1].res)) {
        case true : reading_res = 0; break;
        case false: handles[idx] = INVALID_HANDLE_VALUE; break;
        }
      }
      ...
      switch (RKT_read_stream(handles[idx], &r.capt[i], &caps[i])) {
      case true : break;
      case false: handles[idx] = INVALID_HANDLE_VALUE; break;
      }
    }
