...
static const bool RKT_print_passes;
#ifdef __GNUC__
...
#define RKT_SNDBUF1(RES, e1, FMT1)                                             \
  ...
  RKT_glob.resbuf.len = (RKT_print_passes || RKT_glob.resbuf.res)              \
                          ? sprintf(RKT_glob.resbuf.args, FMT1, e1) + 1        \
                          : 0;                                                 \
  return RKT_send_res(&RKT_glob.resbuf);
#define RKT_SNDBUF2(RES, e1, FMT1, e2, FMT2)                                   \
  RKT_glob.resbuf.res = (RES);                                                 \
  RKT_glob.resbuf.len                                                          \
      = (RKT_print_passes || RKT_glob.resbuf.res)                              \
...
#define RKT_SNDBUF3(RES, e1, FMT1, e2, FMT2, e3, FMT3)                         \
  ...
  RKT_glob.resbuf.len                                                          \
      = (RKT_print_passes || RKT_glob.resbuf.res)                              \
          ? sprintf(RKT_glob.resbuf.args, FMT1 "%c" FMT2 "%c" FMT3, e1, '\0',  \
                    e2, '\0', e3)                                              \
                + 1                                                            \
          : 0;                                                                 \
  return RKT_send_res(&RKT_glob.resbuf);
...
rkt_fun size_t RKTf_streq_impl(bool eq, const char* e1, const char* e2) {
  RKT_AssertResPkg* res = &RKT_glob.resbuf;
  size_t l1 = strlen(e1), l2 = strlen(e2), dif = RKT_strdiff(e1, l1, e2, l2);
  size_t r = eq ? dif : !(dif != 0);
  if (RKT_print_passes || r) {
...
rkt_fun size_t RKTf_memeq_impl(bool eq, const void* e1, const void* e2,
                               size_t e3) {
  size_t res = !(!e3 || !memcmp(e1, e2, e3));
  if (!eq) { res = 1 - res; }
  RKT_SNDBUF3(res, e1, "%p", e2, "%p", e3, "%zu");
}
...
rkt_fun size_t RKTf_streq_n_impl(bool eq, const char* e1, const char* e2,
                                 size_t n) {
  RKT_AssertResPkg* res = &RKT_glob.resbuf;
  if (!n) {
    res->res = !eq, res->len = 0;
    return RKT_send_res(res);
  }
...
#if 0 
rkt_fun size_t RKT_print_assertresold(const RKT_TestEntry* RKT_restrict e,
                                      RKT_AssertDat                     cur,
                                      FILE* RKT_restrict                out) {
# define RKT_printstub(_S, _FMT, _E, _HDR, _SUFF)                              \
  ...
  case RKT_ASSERTRES_PASS:
    if (RKT_resolve_verbosity(e, RK_OUTPUTSTREAMS_LOG) == RK_VERBOSITY_ALWAYS) {
      return RKT_printstub(out, RKT_FMT_P, e, hdr, "\n");
    }
    return 0;
  ...
  case RKT_ASSERTRES_TIMEOUT:
    return !hdr->pos ? RKT_fputs(RK_INRED("[T] TEST TIMEOUT\n"), out)
                     : RKT_printstub(out, RKT_FMT_T, e, hdr, "\n");
  default: {
    const char* args[6];
    size_t      nargs = RKT_unpack_args(res->args, res->len, args);
    RKT_printstub(out, RKT_FMT_F, e, hdr, ": ");
    switch (hdr->F) {
    case RKTF_true   : return RKT_fputs(RK_INRED(" == 'false'\n"), out);
    case RKTF_false  : return RKT_fputs(RK_INRED(" == 'true'\n"), out);
    case RKTF_null   : return fprintf(out, RK_INRED("%s != 'NULL'\n"), args[0]);
    case RKTF_nonnull: return RKT_fputs(RK_INRED(" == 'NULL'\n"), out);
    case RKTF_eq:
    case RKTF_floateq_tol:
    case RKTF_stdouteq:
    case RKTF_stderreq:
      return fprintf(out, "%s " RK_INRED("!=") " %s\n", args[0], args[1]);
    case RKTF_streq:
    case RKTF_streq_n:
      return fprintf(out, "%s " RK_INRED("!=") " %s (position %zu)\n", args[0],
                     args[1], res->res - 1);
    case RKTF_neq:
    ...
    case RKTF_strneq_n:
      return fprintf(out, "%s " RK_INRED("==") " %s\n", args[0], args[0]);
    case RKTF_lt:
      return fprintf(out, RK_INRED("%s%s%s\n"), args[0],
                     res->res == 1 ? " == " : " > ", args[1]);
    case RKTF_leq: return fprintf(out, RK_INRED("%s > %s"), args[0], args[1]);
    case RKTF_gt:
      return fprintf(out, RK_INRED("%s%s%s\n"), args[0],
                     res->res == 1 ? " == " : " < ", args[1]);
    case RKTF_geq: return fprintf(out, RK_INRED("%s < %s"), args[0], args[1]);
    case RKTF_inrange:
      return fprintf(out, RK_INRED("%s%s%s\n"), args[0],
                     res->res == 1 ? " < " : " > ",
                     res->res == 1 ? args[1] : args[2]);
    case RKTF_memeq:
      return fprintf(out, "%s and %s (%s bytes long) " RK_INRED("not equal\n"),
                     args[0], args[1], args[2]);
    case RKTF_memneq:
      return fprintf(out, "%s and %s (%s bytes long) " RK_INRED("equal\n"),
                     args[0], args[1], args[2]);
    case RKTF_memzero:
      return fprintf(out,
                     "%s (%s bytes long) " RK_INRED("nonzero at byte %zu\n"),
                     args[0], args[1], res->res - 1);
    case RKTF_memnzero:
      return fprintf(out, "%s (%s bytes long) " RK_INRED("zero at byte %zu\n"),
                     args[0], args[1], res->res - 1);
    case RKTF_crash:
      if (res->res == 2) {
        return RKT_fputs("Did not crash\n", out);
      } else {
        return fprintf(out, "Expected: %s, got: %s\n",
                       RKT_strcrash(hdr->reason),
                       RKT_strcrash(e->res.exit_status.reason));
      }
    case RKTF_exit:
      if (res->res == 2) {
        return RKT_fputs("Did not exit\n", out);
      } else {
        return fprintf(out,
                       "Expected: exit via code %d, got: exit with code %d\n",
...
#define RKT_printstub(_S, _FMT, _E, _HDR, _SUFF)                               \
  ...
  const char* f;
  switch (hdr->F) {
  case RKTF_true       : f = RK_INRED(" == 'false'\n"); break;
  case RKTF_false      : f = RK_INRED(" == 'true'\n"); break;
  case RKTF_null       : f = RK_INRED("%s != 'NULL'\n"); break;
  case RKTF_nonnull    : f = RK_INRED(" == 'NULL'\n"); break;
  case RKTF_eq         :
  case RKTF_floateq_tol:
  case RKTF_stdouteq   :
  case RKTF_stderreq   : f = "%s " RK_INRED("!=") " %s\n"; break;
  case RKTF_streq:
  case RKTF_streq_n:
    f = "%s " RK_INRED("!=") " %s (position %s)\n";
  ...
  case RKTF_gt:
    f = res->res == 1 ? "%s " RK_INRED("==") " %s\n"
                      : "%s " RK_INRED("<") " %s\n";
    break;
  case RKTF_leq: f = RK_INRED("%s > %s"); break;
  case RKTF_geq: f = "%s < %s"; break;
  case RKTF_inrange:
    f       = res->res == 1 ? "%s " RK_INRED("<") " %s\n"
                            : "%s " RK_INRED(">") " %s\n";
    ...
    break;
  case RKTF_memeq:
    f = "%s and %s (%s bytes long) " RK_INRED("not equal\n");
    break;
  case RKTF_memneq: f = "%s and %s (%s bytes long) " RK_INRED("equal\n"); break;
  case RKTF_memzero:
    f = "%s (%s bytes long) " RK_INRED("nonzero at byte %zu\n");
    sprintf(numbuf[0], "%zu", res->res - 1), args[2] = numbuf[0];
    break;
  case RKTF_memnzero:
    f = "%s (%s bytes long) " RK_INRED("zero at byte %zu\n");
    sprintf(numbuf[0], "%zu", res->res - 1), args[2] = numbuf[0];
    break;
  case RKTF_crash:
    if (res->res == 2) {
  ...
  case RKTF_exit:
    if (res->res == 2) {
      ...
      f = "Expected: exit via code %d, got: exit with code %d\n";
...
rkt_fun void RKT_print_test_reg(const RKT_TestEntry* RKT_restrict e) {
  ...
  case RKT_FAILED:
    ...
    assert(r.count >= r.fails && "failed");

    break;
  case RKT_CRASHED:
    assert(r.count >= r.fails && "crashed");
  ...
  case RKT_EXITED:
    ...
    break;
  case RKT_TIMEDOUT:
    assert(r.count >= r.fails && "timedout");
    ...
    break;
  case RKT_TESTERROR:
    assert(r.count >= r.fails && "testerr");
...
rkt_fun void RKT_run_all(const char** suites_strs, size_t nsuites_strs,
  ...
  if (!suites_strs && nsuites_strs) {
    RKT_fatal("NULL suites_strs requires zero nsuites_strs");
  }
...
rkt_fun void RKT_parent_loop_noisolation(RKT_TestEntry* RKT_restrict e,
  ...
  for (bool reading_res = 0; cond == -1; reading_res = !reading_res) {
    if (!reading_res) {
      ...
      case -2: ++r->count, cond = 1; break; // timed out
      ...
      case 1 : r->ended = RKT_FAILED; r->fails += ad->res.res > 0;
      }
    }
  }
    ...
    *exit = RKT_reapchild();
    if (exit_condition == 3) { // during assert
      ...
      if (exit->type == RKT_EXIT_FAULT) { // child crashed
        if (hdr->F != RKTF_crash) {       // unexpectedly
          res->res = RKT_ASSERTRES_CRASH, *end = RKT_CRASHED;
        } else {
          if (hdr->reason != RKT_FAULT_ANY && hdr->reason != exit->reason) {
            res->res = 1;
            //  ++r->fails; // todo
          } else {
            res->res = 0;
        ...
        if (hdr->F != RKTF_exit) { // unexpectedly
          res->res = RKT_ASSERTRES_EXIT, *end = RKT_EXITED;
        } else {
          if (hdr->exit_code != -1 && hdr->exit_code != exit->exit_code) {
            res->res = 1;
            // ++r->fails; // todo
          } else {
            res->res = 0;
