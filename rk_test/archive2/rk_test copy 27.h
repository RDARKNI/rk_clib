...
rkt_fun size_t RKT_unpack_args(char* restrict args_in, size_t len,
  ...
  if (!args_in) { return 0; }
  size_t n = 0;
  char * p = args_in, *end = args_in + len;
  while (p < end && n < 5) {
    args[n++]  = p;             // start of string
    p         += strlen(p) + 1; // skip to next null terminator
  }
  if (p < end) { RKT_fatal("more arguments than allowed"); }
  args[n] = NULL; // sentinel
  return n;
}
