...
rkt_fun int RKT_init_test_noisolation(const RKT_TestEntry* restrict e,
                                      rkt_fd* restrict out_rfds) {
  static int saved_out = 0, saved_err, wfds[3], rfds[3];
  int        timed_out = 0;
  if (!saved_out) {
    ...
    for (int i = 0; i < RKT_COUNTOF(wfds); ++i) {
      if ((wfds[i] = mkstemp(temps[i])) < 0) { RKT_fatal("mkstemp"); }
      rfds[i] = open(temps[i], O_RDONLY);
      unlink(temps[i]);
      if (rfds[i] < 0) { RKT_fatal("open read end"); }
    }
  }
  for (size_t i = 0; i < 3; ++i) { ftruncate(wfds[i], 0); }
  if (dup2(wfds[0], STDOUT_FILENO) < 0) { RKT_fatal("dup2 redirect out"); }
  if (dup2(wfds[1], STDERR_FILENO) < 0) { RKT_fatal("dup2 redirect err"); }
  RKT_glob.log_writer = wfds[2];
  if (e->attrs.init) { e->attrs.init(); }
  ...
  for (size_t i = 0; i < 3; ++i) {
    if (lseek(wfds[i], 0, SEEK_SET) == (off_t)-1) { RKT_fatal("seek wfds"); }
    if (lseek(rfds[i], 0, SEEK_SET) == (off_t)-1) { RKT_fatal("seek rfds"); }
    out_rfds[i] = rfds[i];
  }
  return timed_out;
}
