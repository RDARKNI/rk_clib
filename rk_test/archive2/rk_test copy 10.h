...
static struct {
  ...
  rk_fd             RK__OUT;
  union {
    RKT_AssertResPkg RKT_UNUSED_align;
...
rkt_fun void RKT_print_test_tap(const RKT_TestEntry* e) {
  ...
  case RKT_FAILED:
    fprintf(out, "%zu fails, %zu passed (total %zu)\n", e->res.fails,
  ...
  case RKT_TESTERROR: // todo windows
    ...
    if (WIFSIGNALED(e->res.term_code)) {
      fprintf(out, RK_INRED("Error in Test Function; Crashed with signal %d"),
  ...
  for (int nfds = 3; nfds > 0;) {
    ...
    if (handles[idx] == pipes[RK_OUTPUTSTREAMS_META]) {
      if (!reading_res) {
        ...
        switch (RKT_read_hdr(handles[idx], &r.results[r.count - 1].res)) {
        case true : reading_res = 1; break;
