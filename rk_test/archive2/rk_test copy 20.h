...
// clang-format off
#define rk_expect_true(EXPR_)                 AE(expect, RKTf_true, RKTF_TRUE, "true("#EXPR_")", EXPR_)
#define rk_expect_false(EXPR_)                AE(expect, RKTf_false, RKTF_FALSE, "false("#EXPR_")", EXPR_)
#define rk_expect_null(PTR_)                  AE(expect, RKTf_null, RKTF_NULL, "null("#PTR_")", PTR_)
#define rk_expect_nonnull(PTR_)               AE(expect, RKTf_nnull, RKTF_NNULL, "nonnull("#PTR_")", PTR_)
#define rk_expect_memeq(ptr1, ptr2, siz)      AE(expect, RKTf_memeq, RKTF_MEMEQ, "memeq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_expect_memneq(ptr1, ptr2, siz)     AE(expect, RKTf_memneq, RKTF_MEMNEQ, "memneq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_expect_memzero(ptr, siz)           AE(expect, RKTf_memzero, RKTF_MEMZERO, "memzero(" #ptr ",  " #siz ")", ptr, siz)
#define rk_expect_memnzero(ptr, siz)          AE(expect, RKTf_memnzero, RKTF_MEMNZERO, "memnzero(" #ptr ",  " #siz ")", ptr, siz)
#define rk_expect_eq(exp, act)                AE(expect, RKT_SELFUN(eq, exp), RKTF_EQ, "eq("#exp", " #act")", exp, act)
#define rk_expect_neq(exp, act)               AE(expect, RKT_SELFUN(neq, exp), RKTF_NEQ, "neq("#exp", " #act")", exp, act)
#define rk_expect_lt(exp, act)                AE(expect, RKT_SELFUN(eq, exp), RKTF_LT, "lt("#exp", " #act")", exp, act)
#define rk_expect_leq(exp, act)               AE(expect, RKT_SELFUN(neq, exp), RKTF_LEQ, "leq("#exp", " #act")", exp, act)
#define rk_expect_gt(exp, act)                AE(expect, RKT_SELFUN(eq, exp), RKTF_GT, "gt("#exp", " #act")", exp, act)
#define rk_expect_geq(exp, act)               AE(expect, RKT_SELFUN(neq, exp), RKTF_GEQ, "geq("#exp", " #act")", exp, act)
#define rk_expect_inrange(val, low, high)     AE(expect, RKT_SELFUN(inrange, val), RKTF_INRANGE, "inrange(" #val ", " #low", " #high ")", val, low, high)
#define rk_expect_floateq_tol(exp, act, tol)  AE(expect, RKT_SELFUN_TOL(toleq, exp), RKTF_TOLEQ, "floateq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define rk_expect_floatneq_tol(exp, act, tol) AE(expect, RKT_SELFUN_TOL(tolneq, exp), RKTF_TOLNEQ,  "floatneq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define rk_expect_streq(str1, str2)           AE(expect, RKTf_streq, RKTF_STREQ, "streq(" #str1 ", " #str2 ")", str1, str2)
#define rk_expect_strneq(str1, str2)          AE(expect, RKTf_strneq, RKTF_STRNEQ,"strneq(" #str1 ", " #str2 ")", str1, str2)                                    
#define rk_expect_streq_n(str1, str2, len)    AE(expect, RKTf_streq_n, RKTF_STREQ, "streq(" #str1 ", " #str2 ", " #len")", str1, str2, len)
#define rk_expect_strneq_n(str1, str2, len)   AE(expect, RKTf_strneq_n, RKTF_STRNEQ,"strneq(" #str1 ", " #str2 ", " #len")", str1, str2, len)  
#define rk_expect_stdouteq(errstr, len)       AE(expect, RKTf_streameq, RKTF_STDOUTEQ, "stdouteq(" #errstr ")", RK_OUTPUTSTREAMS_STDOUT, errstr, len)
#define rk_expect_stdoutneq(errstr, len)      AE(expect, RKTf_streamneq, RKTF_STDOUTNEQ, "stdoutneq(" #errstr ")",  RK_OUTPUTSTREAMS_STDOUT, errstr, len)
#define rk_expect_stderreq(errstr, len)       AE(expect, RKTf_streameq, RKTF_STDERREQ, "stderreq(" #errstr ")", RK_OUTPUTSTREAMS_STDERR, errstr, len)
#define rk_expect_stderrneq(errstr, len)      AE(expect, RKTf_streamneq, RKTF_STDERRNEQ, "stderrneq(" #errstr ")",  RK_OUTPUTSTREAMS_STDERR, errstr, len)

#define rk_assert_true(expr)                  AE(assert, RKTf_true, RKTF_TRUE, "true("#expr")", expr)
#define rk_assert_false(expr)                 AE(assert, RKTf_false, RKTF_FALSE, "false("#expr")", expr)
#define rk_assert_null(ptr)                   AE(assert, RKTf_null, RKTF_NULL, "null("#ptr")", ptr)
#define rk_assert_nonnull(ptr)                AE(assert, RKTf_nnull, RKTF_NNULL, "nonnull("#ptr")", ptr)
#define rk_assert_memeq(ptr1, ptr2, siz)      AE(assert, RKTf_memeq, RKTF_MEMEQ, "memeq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_assert_memneq(ptr1, ptr2, siz)     AE(assert, RKTf_memneq, RKTF_MEMNEQ, "memneq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_assert_memzero(ptr, siz)           AE(assert, RKTf_memzero, RKTF_MEMZERO, "memzero(" #ptr ",  " #siz ")", ptr, siz)
#define rk_assert_memnzero(ptr, siz)          AE(assert, RKTf_memnzero, RKTF_MEMNZERO, "memnzero(" #ptr ",  " #siz ")", ptr, siz)
#define rk_assert_eq(exp, act)                AE(assert, RKT_SELFUN(eq, exp), RKTF_EQ, "eq("#exp", " #act")", exp, act)
#define rk_assert_neq(exp, act)               AE(assert, RKT_SELFUN(neq, exp), RKTF_NEQ, "neq("#exp", " #act")", exp, act)
#define rk_assert_lt(exp, act)                AE(assert, RKT_SELFUN(eq, exp), RKTF_LT, "lt("#exp", " #act")", exp, act)
#define rk_assert_leq(exp, act)               AE(assert, RKT_SELFUN(neq, exp), RKTF_LEQ, "leq("#exp", " #act")", exp, act)
#define rk_assert_gt(exp, act)                AE(assert, RKT_SELFUN(eq, exp), RKTF_GT, "gt("#exp", " #act")", exp, act)
#define rk_assert_geq(exp, act)               AE(assert, RKT_SELFUN(neq, exp), RKTF_GEQ, "eq("#exp", " #act")", exp, act)
#define rk_assert_inrange(val, low, high)     AE(assert, RKT_SELFUN(inrange, val), RKTF_INRANGE, "inrange(" #val ", " #low", " #high ")", val, low, high)
#define rk_assert_floateq_tol(exp, act, tol)  AE(assert, RKT_SELFUN_TOL(toleq, exp), RKTF_TOLEQ, "floateq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define rk_assert_floatneq_tol(exp, act, tol) AE(assert, RKT_SELFUN_TOL(tolneq, exp), RKTF_TOLNEQ,  "floatneq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define rk_assert_streq(str1, str2)           AE(assert, RKTf_streq, RKTF_STREQ, "streq(" #str1 ", " #str2 ")", str1, str2)
#define rk_assert_strneq(str1, str2)          AE(assert, RKTf_strneq, RKTF_STRNEQ,"strneq(" #str1 ", " #str2 ")", str1, str2)  
#define rk_assert_streq_n(str1, str2, len)    AE(assert, RKTf_streq_n, RKTF_STREQ, "streq(" #str1 ", " #str2 ", " #len")", str1, str2, len)
#define rk_assert_strneq_n(str1, str2,len)    AE(assert, RKTf_strneq_n, RKTF_STRNEQ,"strneq(" #str1 ", " #str2 ", " #len")", str1, str2, len)  
#define rk_assert_stdouteq(errstr, len)       AE(assert, RKTf_streameq, RKTF_STDOUTEQ, "stdouteq(" #errstr ")", RK_OUTPUTSTREAMS_STDOUT, errstr, len)
#define rk_assert_stdoutneq(errstr, len)      AE(assert, RKTf_streamneq, RKTF_STDOUTNEQ, "stdoutneq(" #errstr ")", RK_OUTPUTSTREAMS_STDOUT, errstr, len)
#define rk_assert_stderreq(errstr, len)       AE(assert, RKTf_streameq, RKTF_STDERREQ, "stderreq(" #errstr ")", RK_OUTPUTSTREAMS_STDERR, errstr, len)
#define rk_assert_stderrneq(errstr, len)      AE(assert, RKTf_streamneq, RKTF_STDERRNEQ, "stderrneq(" #errstr ")", RK_OUTPUTSTREAMS_STDERR, errstr, len)


#define RKT_AE_true(AE, expr)                  RKT_AE(AE, RKTf_true, RKTF_TRUE, "true("#expr")", expr)
#define RKT_AE_false(AE, expr)                 RKT_AE(AE, RKTf_false, RKTF_FALSE, "false("#expr")", expr)
#define RKT_AE_null(AE, ptr)                   RKT_AE(AE, RKTf_null, RKTF_NULL, "null("#ptr")", ptr)
#define RKT_AE_nonnull(AE, ptr)                RKT_AE(AE, RKTf_nnull, RKTF_NNULL, "nonnull("#ptr")", ptr)
#define RKT_AE_memeq(AE, ptr1, ptr2, siz)      RKT_AE(AE, RKTf_memeq, RKTF_MEMEQ, "memeq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define RKT_AE_memneq(AE, ptr1, ptr2, siz)     RKT_AE(AE, RKTf_memneq, RKTF_MEMNEQ, "memneq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define RKT_AE_memzero(AE, ptr, siz)           RKT_AE(AE, RKTf_memzero, RKTF_MEMZERO, "memzero(" #ptr ",  " #siz ")", ptr, siz)
#define RKT_AE_memnzero(AE, ptr, siz)          RKT_AE(AE, RKTf_memnzero, RKTF_MEMNZERO, "memnzero(" #ptr ",  " #siz ")", ptr, siz)
#define RKT_AE_eq(AE, exp, act)                RKT_AE(AE, RKT_SELFUN(eq, exp), RKTF_EQ, "eq("#exp", " #act")", exp, act)
#define RKT_AE_neq(AE, exp, act)               RKT_AE(AE, RKT_SELFUN(neq, exp), RKTF_NEQ, "neq("#exp", " #act")", exp, act)
#define RKT_AE_lt(AE, exp, act)                RKT_AE(AE, RKT_SELFUN(eq, exp), RKTF_LT, "lt("#exp", " #act")", exp, act)
#define RKT_AE_leq(AE, exp, act)               RKT_AE(AE, RKT_SELFUN(neq, exp), RKTF_LEQ, "leq("#exp", " #act")", exp, act)
#define RKT_AE_gt(AE, exp, act)                RKT_AE(AE, RKT_SELFUN(eq, exp), RKTF_GT, "gt("#exp", " #act")", exp, act)
#define RKT_AE_geq(AE, exp, act)               RKT_AE(AE, RKT_SELFUN(neq, exp), RKTF_GEQ, "eq("#exp", " #act")", exp, act)
#define RKT_AE_inrange(AE, val, low, high)     RKT_AE(AE, RKT_SELFUN(inrange, val), RKTF_INRANGE, "inrange(" #val ", " #low", " #high ")", val, low, high)
#define RKT_AE_floateq_tol(AE, exp, act, tol)  RKT_AE(AE, RKT_SELFUN_TOL(toleq, exp), RKTF_TOLEQ, "floateq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define RKT_AE_floatneq_tol(AE, exp, act, tol) RKT_AE(AE, RKT_SELFUN_TOL(tolneq, exp), RKTF_TOLNEQ,  "floatneq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define RKT_AE_streq(AE, str1, str2)           RKT_AE(AE, RKTf_streq, RKTF_STREQ, "streq(" #str1 ", " #str2 ")", str1, str2)
#define RKT_AE_strneq(AE, str1, str2)          RKT_AE(AE, RKTf_strneq, RKTF_STRNEQ,"strneq(" #str1 ", " #str2 ")", str1, str2)  
#define RKT_AE_streq_n(AE, str1, str2, len)    RKT_AE(AE, RKTf_streq_n, RKTF_STREQ, "streq(" #str1 ", " #str2 ", " #len")", str1, str2, len)
#define RKT_AE_strneq_n(AE, str1, str2,len)    RKT_AE(AE, RKTf_strneq_n, RKTF_STRNEQ,"strneq(" #str1 ", " #str2 ", " #len")", str1, str2, len)  
#define RKT_AE_stdouteq(AE, errstr, len)       RKT_AE(AE, RKTf_streameq, RKTF_STDOUTEQ, "stdouteq(" #errstr ")", RK_OUTPUTSTREAMS_STDOUT, errstr, len)
#define RKT_AE_stdoutneq(AE, errstr, len)      RKT_AE(AE, RKTf_streamneq, RKTF_STDOUTNEQ, "stdoutneq(" #errstr ")", RK_OUTPUTSTREAMS_STDOUT, errstr, len)
#define RKT_AE_stderreq(AE, errstr, len)       RKT_AE(AE, RKTf_streameq, RKTF_STDERREQ, "stderreq(" #errstr ")", RK_OUTPUTSTREAMS_STDERR, errstr, len)
#define RKT_AE_stderrneq(AE, errstr, len)      RKT_AE(AE, RKTf_streamneq, RKTF_STDERRNEQ, "stderrneq(" #errstr ")", RK_OUTPUTSTREAMS_STDERR, errstr, len)

#define RKT_AE(TYP, FUN, FUNENUM, EXPRSTR, ...) RKT_CONCAT(RKT_, TYP)(FUN, FUNENUM, #TYP "_" EXPRSTR, __VA_ARGS__)
...
#define RKT_expect(FUN, FUNENUM, EXPRSTR, ...)                                 \
  (RKT_SEND_HDR(EXPRSTR, FUNENUM, 0), FUN(__VA_ARGS__))

#define RKT_assert(FUN, FUNENUM, EXPRSTR, ...)                                 \
...
#define RKT_SEND_HDR(ESTR, FUNENUM, SIG)                                       \
  RKT_send_hdr(                                                                \
      (RKT_AssertHdr){__LINE__, FUNENUM, {SIG}, {ESTR, RKT_lenof(ESTR)}})
...
// todo
rkt_fun size_t RKTf_streameq(RK_OutPutStreams stream, const char* str,
                             size_t len) {
  fflush(NULL);
  ...
  res->res = dif, res->len = len + o.len + 2;
...
rkt_fun size_t RKTf_streamneq(RK_OutPutStreams stream, const char* str,
                              size_t len) {
  fflush(NULL);
  RKT_Strv          o = RKT_readfile(RKT_glob.tmpfds[stream]);
  RKT_AssertResPkg* res;
  ...
  res->res = !(dif != 0), res->len = len + o.len + 2;
  if (len) { memcpy(res->args, str, len); }
  res->args[len] = '\0';
  if (o.len) { memcpy(res->args + len + 1, o.str, o.len); }
  res->args[len + 1 + o.len] = '\0';
...
rkt_fun void RKT_run_suite(RKT_Suite* s) {
  ...
  for (RKT_TestEntry* e = s->tests; e; e = e->next) {
    ...
    const char *s1, *e1;
    for (s1 = RKT_glob.attrs.tags, e1 = s1; *s1; s1 = *e1 ? e1 + 1 : e1) {
      for (e1 = s1; *e1 && *e1 != ','; ++e1);
      const char *s2, *e2;
      for (s2 = e->attrs.tags, e2 = s2; *s2; s2 = *e2 ? e2 + 1 : e2) {
        for (e2 = s2; *e2 && *e2 != ','; ++e2);
