...
typedef void (*RK_TestFunc)(void); // todo use
...
rkt_fun int             RKT_read_full_nb(rkt_fd fd, void* buf, size_t nbytes);
rkt_fun rkt_pid         RKT_init_test(const RKT_TestEntry* restrict e,
                                      rkt_fd* restrict pipes);
rkt_fun RKT_TestEndType RKT_parent_loop(RKT_TestEntry* restrict e, rkt_pid pid,
                                        rkt_fd* restrict pipes);
...
rkt_fun RKT_Exit        RKT_inspect_exit(int code) {
  ...
  if (WIFSIGNALED(code)) {
    ...
    res.crashed = false, res.exit_code = WEXITSTATUS(code);
  }
#else
  if (code >= 0x80000000) {
    res.crashed = true;
    ...
    res.crashed = false, res.exit_code = code;
  }
#endif
  return res;
}
...
rkt_fun void RKT_print_test_tap(const RKT_TestEntry* e) {
  ...
  case RKT_TESTERROR: // todo windows
    fputs(RK_INRED("Error in Test Function: "), out);
    if (r.exit_status.crashed) {
...
rkt_fun RKT_TestEndType RKT_run_test(RKT_TestEntry* e) {
  ...
  if (RKT_resolve_isolation(e) == false) {
    ...
    rkt_pid pid = RKT_init_test(e, pipes);
...
rkt_fun void RKT_run_suite(RKT_Suite* s) {
  ...
  if (s->attrs.fini) { s->attrs.fini(); } // todo windows should skip this
...
rkt_fun void RKT_init(RK_CustomTestAttributes attrs) {
  for (int i = 0; i < 3; ++i) {
  ...
  for (ssize_t r; nbytes;) {
    ...
    case -1:
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        poll(&pfd, 1, 10000); // todo maybe timeout
      } else if (errno != EINTR) {
        return -1;
      }
      break;
...
rkt_fun int RKT_read_hdr(rkt_fd fd, RKT_AssertHdr* restrict hdr) {
  return RKT_read_full_nb(fd, hdr, sizeof(*hdr));
}

rkt_fun int RKT_read_res(rkt_fd fd, RKT_AssertRes* restrict res) {
  ...
  case 1:
    if (!res->len) {
      ...
      if (!(res->args[0] = RKT_malloc(char, res->len))) {
        RKT_fatal("malloc read res");
      }
...
rkt_fun bool RKT_read_stream(rkt_fd fd, RKT_Strv* str, size_t* restrict cap) {
  if (str->len == *cap - 1) {
...
rkt_fun RKT_Exit RKT_reapchild(rkt_pid pid) {
#ifndef _WIN32
  int status;
...
rkt_fun void RKT_finalise_test(RKT_TestResult* r, rkt_pid pid,
                               bool exit_during_assert) {
  r->ended       = RKT_PASSED;
  r->exit_status = RKT_reapchild(pid);
  if (exit_during_assert) {
    const RKT_AssertHdr* hdr = &r->results[r->count - 1].hdr;
    RKT_AssertRes*       res = &r->results[r->count - 1].res;
    memset(res, 0, sizeof(*res));
    if (r->exit_status.crashed) {
      if (hdr->F != RKTF_CRASH) {
        res->res = RKT_ASSERTRES_CRASH, r->ended = RKT_CRASHED;
      } else if (hdr->F == RKTF_CRASH) {
        res->res = (hdr->reason != RKT_SIGNAL_ANY
                    && hdr->reason != r->exit_status.reason);
      }
    } else {
      if (hdr->F != RKTF_EXIT) {
        res->res = RKT_ASSERTRES_EXIT, r->ended = RKT_EXITED;
      } else {
    ...
    if (res->res) { ++r->fails; }
  } else if (r->exit_status.crashed || r->exit_status.exit_code) {
...
rkt_fun rkt_pid RKT_init_test(const RKT_TestEntry* restrict e,
  ...
  if (pid == 0) {
    ...
    int failed_assert   = setjmp(RKT_glob.fret);
    if (failed_assert == 0) {
      e->func();
    } else {
      // todo jump
    ...
    for (int i = 0; i < 3; ++i) { // set pipes nonblocking
      ...
      if (flags == -1 || fcntl(pipes[i], F_SETFL, flags | O_NONBLOCK) == -1) {
        RKT_fatal("fcntl set nonblocking");
      }
    }
  }
  return pid;
...
/*
todo important: windows version
*/

rkt_fun int RKT_init_test_noisolation_win(const RKT_TestEntry* restrict e,
                                          rkt_fd* restrict out_rfds) {}

/*
In isolated:
Reader: three fds completely separate from global state
Writer: Global write fd, redirected stdout/err


In non-isolated:
Log fd is same as global write fd (read write, one fd)
two other fds
*/

rkt_fun int RKT_init_test_noisolation(const RKT_TestEntry* restrict e,
                                      rkt_fd* restrict out_fds) {
  static int saved_out, saved_err, fds[3];
  if (!saved_out) {
    if ((saved_out = dup(STDOUT_FILENO)) < 0) { RKT_fatal("dup save out"); }
    if ((saved_err = dup(STDERR_FILENO)) < 0) { RKT_fatal("dup save err"); }
    char temps[3][32]
  ...
  for (size_t i = 0; i < 3; ++i) {
    if (ftruncate(fds[i], 0) < 0) { RKT_fatal("ftruncate"); }
    if (lseek(fds[i], 0, SEEK_SET) == (off_t)-1) { RKT_fatal("seek wfds"); }
  }
  if (dup2(fds[0], STDOUT_FILENO) < 0) { RKT_fatal("dup2 redirect out"); }
  if (dup2(fds[1], STDERR_FILENO) < 0) { RKT_fatal("dup2 redirect err"); }
  ...
  if (!timed_out) {
    int assert_failed = setjmp(RKT_glob.fret);
    if (!assert_failed) {
      e->func();
    } else {
      // failed assertion
  ...
  if (e->attrs.fini) { e->attrs.fini(); }
  if (dup2(saved_out, STDOUT_FILENO) < 0) { RKT_fatal("dup2 restore out"); }
  if (dup2(saved_err, STDERR_FILENO) < 0) { RKT_fatal("dup2 restore err"); }
  fflush(NULL);
  for (size_t i = 0; i < 3; ++i) {
    if (lseek(fds[i], 0, SEEK_SET) == (off_t)-1) { RKT_fatal("seek rfds"); }
    out_fds[i] = fds[i];
  }
  return timed_out;
}
...
# endif
  ...
  switch (RKT_read_full_nb(fd, res.str, res.len)) {
...
rkt_fun RKT_TestEndType RKT_parent_loop_noisolation(RKT_TestEntry* restrict e,
  ...
  e->res.capt[RK_OUTPUTSTREAMS_STDOUT]
      = RKT_readfile(fds[RK_OUTPUTSTREAMS_STDOUT]);
  e->res.capt[RK_OUTPUTSTREAMS_STDERR]
      = RKT_readfile(fds[RK_OUTPUTSTREAMS_STDERR]);
  e->res.results = RKT_malloc(RKT_AssertDat, rescap);
  if (!e->res.results) { RKT_fatal("malloc buf"); }
  for (enum {READHDR, READRES, DONE} state = READHDR; state != DONE;) {
    if (state == READHDR) {
      ...
      case -2: state = DONE; break; // incomplete read
      case -1: RKT_fatal("read hdr");
      case 0 : state = DONE; break;
      case 1 : ++e->res.count, state = READRES;
      }
    } else {
  ...
  if (timed_out) { e->res.ended = RKT_TIMEDOUT; }
  return e->res.ended;
}
  ...
  for (int nfds = 3; nfds > 0;) {
    for (;;) {
      ...
      if (pr == -1) {
        ...
        if (!reading_res) { // todo why increase count?
  ...
  for (RKT_Suite* s = RKT_glob.suites; s; s = s->next) {
    for (RKT_TestEntry* e = s->tests; e; e = e->next) {
      if (!strcmp(buf, e->name.str)) {
        ...
        char*  ep;
        rkt_fd meta_fd = (rkt_fd)(uintptr_t)strtoumax(buf, &ep, 10);
        if (*ep != '\0') { exit(1); }
        if (RKT_glob.attrs.init) { RKT_glob.attrs.init(); }
        if (s->attrs.init) { s->attrs.init(); }
        if (e->attrs.init) { e->attrs.init(); }
...
rkt_fun RKT_TestEndType RKT_parent_loop(RKT_TestEntry* restrict e, rkt_pid pid,
  ...
  long   timeout_ms = RKT_resolve_timeout(e), time_start = RKT_now_ms();
  rkt_fd handles[4]  = {pipes[0], pipes[1], pipes[2]};
  bool   reading_res = 0;
  for (int nfds = 3; nfds > 0;) {
