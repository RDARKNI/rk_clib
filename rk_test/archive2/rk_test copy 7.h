...
rkt_fun size_t RKT_fmt_assertres(const RKT_TestEntry* restrict e,
  ...
  case RKT_ASSERTRES_PASS:
    if (RKT_resolve_verbosity(e, RK_OUTPUTSTREAMS_META)
        == RK_VERBOSITY_ALWAYS) {
      s += RK_fmt_stub(s, RK_TFMT_P, e, hdr), *s++ = '\n';
    }
    break;
  case RKT_ASSERTRES_EXIT:
    s += RK_fmt_stub(s, RK_TFMT_E, e, hdr), *s++ = '\n';
    break;
  case RKT_ASSERTRES_CRASH:
    s += RK_fmt_stub(s, RK_TFMT_C, e, hdr), *s++ = '\n';
    break;
  case RKT_ASSERTRES_TIMEOUT:
    if (hdr.pos == 0) {
      s += RK_catlit(s, RK_INRED("[T] TEST TIMEOUT\n"));
    } else {
      s += RK_fmt_stub(s, RK_TFMT_T, e, hdr), *s++ = '\n';
    }
    break;
  default:
    ...
    s += RK_fmt_stub(s, RK_TFMT_F, e, hdr), *s++ = ' ';
    ...
    case RKTF_MEMZERO:
      ...
      break;
    case RKTF_MEMNZERO:
      s += sprintf(s, "%s (%s bytes long) " RK_INRED("zero at byte %s") ".\n",
                   args[0], args[1], res.res - 1);
      break;
    case RKTF_STREQ:
      s += sprintf(s,
