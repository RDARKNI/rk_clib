...
rkt_fun void RKT_unpack_args(char* restrict args_in, size_t len, char** args) {
  memset(args, 0, sizeof(char*) * 6);
  args[0] = args_in;
  if (args_in) {
    size_t i = 1;
    for (size_t len = 0; len + 1 < len; ++len) {
      if (!args[0][len]) {
        assert(i + 1 < 6);
        args[i++] = args[0] + len + 1;
      }
  ...
  if (!r->count) {
    ...
    for (size_t i = 0; i < r->count; ++i) {
      ...
      fprintf(out,
              ...
              resstr);
      char* args[6];
      RKT_unpack_args(ad->res.args, ad->res.len, args);

      /* args (max 3) */
      for (int j = 0; j < 3; ++j) {
        const char* a0 = args[j];
        const char* a1 = args[j + 1];
        if (!a1) { break; }
        if (j) { RKT_fputs(", ", out); }
        fputc('"', out);
...
rkt_fun int RKT_read_res(RKT_AssertRes* restrict res, long ms_left) {
  ...
  case 1:
    res->args = NULL;
    if (!res->len) { break; }
    res->args = RKT_malloc(char, res->len);
    if (!res->args) { RKT_fatal("malloc read res"); }
    if (RKT_read_full_tm(RKT_glob.log, res->args, res->len, ms_left) < 1) {
      return -2;
    }
  }
  return 1;
...
rkt_fun RKT_TestEndType RKT_finalise_test(RKT_TestResult* r,
  ...
  r->capt[RK_OUTPUTSTREAMS_STDOUT]
      = RKT_readfile(RKT_glob.tmpfds[RK_OUTPUTSTREAMS_STDOUT]);
  r->capt[RK_OUTPUTSTREAMS_STDERR]
