...
RKT_noreturn rkt_fun void RKT_fatal(rk_pid pid, const char* str);
...
public:
  RK_TestStream() : std::ostream(&streambuf) {} // todo this is cursed
  /*
  todo this shit
  */
  template <class T>
...
rkt_fun size_t RK_fmt_stub(char* restrict s, const RKT_TestEntry* restrict e,
  ...
  size_t l  = strlen(hdr.exprstr);
  s        += RK_catstr(s, hdr.exprstr, l);
  return s - _s;
}
...
rkt_fun void RKT_json_escape(const char* s) {
...
rkt_fun void RKT_json_escape_n(RKT_Strv sv) {
  FILE* out = RKT_glob.output_types[RK_OUTPUTFORMAT_JSON];
    ...
    if (!r.count) {
      ...
      for (size_t i = 0; i < r.count; i++) {
        ...
        for (int j = 0; j < 3; ++j) { // shit
          ...
          if (j > 0) { fputs(", ", out); }
          fputs("\"", out);
          RKT_Strv arg = {ad->res.args[j],
                          (size_t)(ad->res.args[j + 1] - ad->res.args[j] - 1)};
...
RKT_noreturn rkt_fun void RKT_fatal(rk_pid pid, const char* msg) {
