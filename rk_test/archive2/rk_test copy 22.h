...
rkt_fun void RKT_print_test_json(const RKT_TestEntry* e) {
  ...
  RKT_fputs("\",\n"
            "      \"tags\": \"",
            out);
  RKT_json_escape_n(e->attrs.tags.str, e->attrs.tags.len);
  RKT_fputs("\",\n"
            "      \"result\": {\n",
            out);
  fprintf(out, "        \"ended\": %d,\n", r->ended);
...
rkt_fun void RKT_run_suite(RKT_Suite* s) {
  ...
  for (RKT_TestEntry* e = s->tests; e; e = e->next) {
    if (!RKT_glob.attrs.tags.len) { goto run_test; } // todo local tags
    if (!e->attrs.tags.len) { continue; }
    // const char *s1 = RKT_glob.attrs.tags.str, *e1;
    // RKT_glob.attrs.tags.str
    for (size_t i0 = 0, j0; i0 < RKT_glob.attrs.tags.len; ++i0) {
      for (j0 = i0;
           j0 < RKT_glob.attrs.tags.len && RKT_glob.attrs.tags.str[j0] != ',';
           ++j0);
      for (size_t i1 = 0, j1; i1 < e->attrs.tags.len; ++i1) {
        for (j1 = i1; j1 < e->attrs.tags.len && e->attrs.tags.str[j1] != ',';
             ++j1);
        if (j0 - i0 == j1 - i1
            && !strncmp(RKT_glob.attrs.tags.str + i0, e->attrs.tags.str + i1,
                        j1 - i1)) {
          goto run_test;
        }
      }
...
rkt_fun int main_default(int argc, char* argv[]) {
  RK_CustomTestAttributes attrs;
  for (char **args = argv, *arg = *args; arg != NULL; arg = *++args) {
    if (*arg == '-' && *++arg) {
      if (*arg == '-' && *++arg) {
