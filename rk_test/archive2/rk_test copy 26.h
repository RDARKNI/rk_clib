...
#define RKT_SNDBUF0(RES)                                                       \
  RKT_buf.res.res = (RES), RKT_buf.res.len = 0;                                \
  return RKT_send_res(&RKT_buf.res);
#define RKT_SNDBUF1(RES, e1, FMT1)                                             \
  RKT_buf.res.res = (RES);                                                     \
  RKT_buf.res.len = sprintf(RKT_buf.res.args, FMT1, e1) + 1;                   \
  return RKT_send_res(&RKT_buf.res);
#define RKT_SNDBUF2(RES, e1, FMT1, e2, FMT2)                                   \
  RKT_buf.res.res = (RES);                                                     \
  RKT_buf.res.len                                                              \
      = sprintf(RKT_buf.res.args, FMT1 "%c" FMT2, e1, '\0', e2) + 1;           \
  return RKT_send_res(&RKT_buf.res);
#define RKT_SNDBUF3(RES, e1, FMT1, e2, FMT2, e3, FMT3)                         \
  RKT_buf.res.res = (RES);                                                     \
  RKT_buf.res.len = sprintf(RKT_buf.res.args, FMT1 "%c" FMT2 "%c" FMT3, e1,    \
                            '\0', e2, '\0', e3)                                \
                  + 1;                                                         \
  return RKT_send_res(&RKT_buf.res);
...
rkt_fun size_t RKTf_streq(const char* e1, const char* e2) {
  ...
  if (package_size <= sizeof(RKT_buf)) {
    res = &RKT_buf.res;
  } else {
  ...
  RKT_send_res(res);
  if (package_size > sizeof(RKT_buf)) { free(res); }
  return dif;
}
rkt_fun size_t RKTf_strneq(const char* e1, const char* e2) {
  RKT_AssertResPkg* res;
...
rkt_fun size_t RKTf_streq_n(const char* e1, const char* e2, size_t n) {
  RKT_AssertResPkg* res;
  if (!n) {
    res = &RKT_buf.res, res->res = 0, res->len = 0;
    return RKT_send_res(res);
  }
...
rkt_fun size_t RKTf_strneq_n(const char* e1, const char* e2, size_t n) {
  RKT_AssertResPkg* res;
  if (!n) {
    res = &RKT_buf.res, res->res = 1, res->len = 0;
    return RKT_send_res(res);
  }
  size_t dif          = RKT_strdiff(e1, n, e2, n);
  size_t package_size = sizeof(*res) + n + n + 2;
...
*/
rkt_fun size_t RKT_unpack_args(char* args_in, size_t len, char** args) {
  memset(args + 1, 0, sizeof(*args) * 5);
  size_t i = 0;
  args[0]  = args_in;
  if (args[0]) {
    ++i;
    for (size_t l = 0; l + 1 < len; ++l) {
      if (!args[0][l]) {
        args[i++] = args[0] + l + 1;
        if (i == 5) { RKT_fatal("more arguments than allowed"); }
      }
    }
    args[i]     = args[0] + len;
    args[i + 1] = NULL;
...
rkt_fun void RKT_finalise_test_isolation(RKT_TestResult* r,
  ...
  *end                  = RKT_PASSED;
  if (exit_condition == 1 || exit_condition == 2) { // timeout, no exit status
    ...
    if (exit_condition == 2) { memset(hdr, 0, sizeof(RKT_AssertHdr)); }
    memset(res, 0, sizeof(RKT_AssertRes));
