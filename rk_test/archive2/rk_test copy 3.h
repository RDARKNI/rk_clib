...
#define rk_assert_crash(signal, ...)                                           \
  do {                                                                         \
    ...
    {                                                                          \
      ...
      RKT_send_AssertRes(&RK_PKG, sizeof(RK_PKG));                             \
    }                                                                          \
    exit(0);                                                                   \
  } while (0)
...
static inline RKT_TestEndType RKT_run_test(RKT_TestEntry* e);
...
static inline RKT_TestResult  RKT_parent_loop(rk_pid pid, rk_fd* pipes,
                                              long timeout_ms);
static inline void   RKT_print_test_reg(const RKT_TestEntry* restrict e);
static inline size_t RKT_fmt_assertres(const RKT_TestEntry* restrict e,
                                       RKT_AssertDat cur,
                                       char** restrict fmtbuf,
                                       size_t* restrict fmtcap);

static inline int    RKT_interpret_code(int code) { // todo windows
  if (WIFEXITED(code)) {
...
static inline void RKT_send_AssertHdr(RKT_AssertHdr hdr) {
  if (RKT_write_full(RKT_glob.RK__OUT, &hdr, sizeof(hdr))) {
    exit(1); // todo
  }
}

#define RK_test_sendhdr_sig(EXPRSTR, FUNENUM, sig)                             \
  RKT_send_AssertHdr((RKT_AssertHdr){EXPRSTR, __LINE__, FUNENUM, sig})
...
# define RK_test_GENFUN2_sig(T, N, FMT, name)                                  \
   static inline size_t RKTf_##name##_##N(T e1, T e2)
# define RK_test_GENFUN3_sig(T, N, FMT, name)                                  \
   static inline size_t RKTf_##name##_##N(T e1, T e2, T e3)

#else
# define RKT_SELFUN(FTYPE, VAL)     RKTf_##FTYPE
# define RKT_SELFUN_TOL(FTYPE, ...) RKTf_##FTYPE
# define RK_test_GENFUN2_sig(T, N, FMT, name)                                  \
   static inline size_t RKTf_##name(T e1, T e2)
# define RK_test_GENFUN3_sig(T, N, FMT, name)                                  \
   static inline size_t RKTf_##name(T e1, T e2, T e3)
...
# define RK_CPP_GENFUN(OP, OPNAME, ...)                                        \
   ...
   static inline size_t RKTf_##OPNAME(T e1, U e2) {                            \
     RK_test_streambuf buf;                                                    \
     std::ostream      oss(&buf);                                              \
     RK_test_to_string(oss, e1), RK_test_to_string(oss, e2);                   \
     size_t res = !(e1 OP e2);                                                 \
     buf.write_full(RKT_glob.RK__OUT, (size_t)res);                            \
     return res;                                                               \
   }
...
static inline size_t RKTf_inrange(T e1, U e2, O e3) {
  RK_test_streambuf buf;
  std::ostream      oss(&buf);
...
// TODO specialised function
static inline int RKT_send_AssertRes(const RKT_AssertResPkg* buf, size_t len) {
  if (RKT_write_full(RKT_glob.RK__OUT, buf, len)) {
    exit(1); // todo
  }
  return buf->res;
}

#define construct_send_buf0(RES)                                               \
  RKT_AssertResPkg* _res = (RKT_AssertResPkg*)RKT_glob.resbufr;                \
  _res->res = (RES), _res->len = 0;                                            \
  return RKT_send_AssertRes(_res, sizeof(*_res) + _res->len);
#define construct_send_buf1(RES, arg1, FMT1)                                   \
  RKT_AssertResPkg* _res = (RKT_AssertResPkg*)RKT_glob.resbufr;                \
  _res->res              = (RES);                                              \
  _res->len              = sprintf(_res->args, FMT1, e1) + 1;                  \
  return RKT_send_AssertRes(_res, sizeof(*_res) + _res->len);

#define construct_send_buf2(RES, arg1, FMT1, arg2, FMT2)                       \
  RKT_AssertResPkg* _res = (RKT_AssertResPkg*)RKT_glob.resbufr;                \
  _res->res              = (RES);                                              \
  _res->len = sprintf(_res->args, FMT1 "%c" FMT2, e1, '\0', e2) + 1;           \
  return RKT_send_AssertRes(_res, sizeof(*_res) + _res->len);

#define construct_send_buf3(RES, arg1, FMT1, arg2, FMT2, arg3, FMT3)           \
  RKT_AssertResPkg* _res = (RKT_AssertResPkg*)RKT_glob.resbufr;                \
  _res->res              = (RES);                                              \
  _res->len                                                                    \
...
#define RK_test_GENFUN2(T, N, FMT, name, expr)                                 \
  RK_test_GENFUN2_sig(T, N, FMT, name) {                                       \
    construct_send_buf2((expr), e1, FMT, e2, FMT);                             \
  }
#define RK_test_GENFUN3(T, N, FMT, name, expr)                                 \
  RK_test_GENFUN3_sig(T, N, FMT, name) {                                       \
    construct_send_buf3((expr), e1, FMT, e2, FMT, e3, FMT);                    \
  }
...
static inline size_t RKTf_true(bool e1) {
  construct_send_buf0(!e1);
}
// clang-format on
static inline size_t RKTf_false(bool e1) { construct_send_buf0(!!e1); }
static inline size_t RKTf_null(const void* e1) {
  construct_send_buf1(!(e1 == NULL), e1, "%p");
}
static inline size_t RKTf_nnull(const void* e1) {
  construct_send_buf1(!(e1 != NULL), e1, "%p");
}
static inline size_t RKTf_memeq(const void* e1, const void* e2, size_t e3) {
  size_t _result = !((e1 == e2) || (e1 && e2 && !memcmp(e1, e2, e3)));
  construct_send_buf3(_result, e1, "%p", e2, "%p", e3, "%zu");
}
static inline size_t RKTf_memneq(const void* e1, const void* e2, size_t e3) {
  size_t _result = !!((e1 == e2) || (e1 && e2 && !memcmp(e1, e2, e3)));
  construct_send_buf3(_result, e1, "%p", e2, "%p", e3, "%zu");
}

static inline size_t RKTf_memzero(const void* e1, size_t e2) {
  size_t dif = 0;
  ...
  construct_send_buf2(dif, e1, "%p", e2, "%zu");
}

static inline size_t RKTf_memnzero(const void* e1, size_t e2) {
  const void* l;
  size_t r = (!e2 || !(l = memchr(e1, 0, e2))) ? 0 : ((char*)l - (char*)e1 + 1);
  construct_send_buf2(r, e1, "%p", e2, "%zu");
}

static inline size_t RKTf_streq(const char* e1, const char* e2) {
  ...
  RKT_send_AssertRes(res, sizeof(*res) + res->len);
  if (package_size > sizeof(RKT_glob.resbufr)) { free(res); }
  return dif;
}

static inline size_t RKTf_strneq(const char* e1, const char* e2) {
  RKT_AssertResPkg* res;
  size_t            l1 = strlen(e1), l2 = strlen(e2);
  size_t            dif          = RK_test_strdiff(e1, l1, e2, l2);
  size_t            package_size = sizeof(RKT_AssertResPkg) + l1 + l2 + 2;
...
static inline size_t RKTf_streq_n(const char* e1, const char* e2, size_t n) {
  RKT_AssertResPkg* res;
  if (!n) {
    res = (RKT_AssertResPkg*)RKT_glob.resbufr, res->res = 0, res->len = 0;
    RKT_write_full(RKT_glob.RK__OUT, res, sizeof(*res) + res->len);
    return 0;
  }
  ...
  memcpy(res->args, e1, n);
  res->args[n] = '\0';
  memcpy(res->args + n + 1, e2, n);
  res->args[n + 1 + n] = '\0';
  RKT_send_AssertRes(res, sizeof(*res) + res->len);
  if (package_size > sizeof(RKT_glob.resbufr)) { free(res); }
  return dif;
...
static inline size_t RKTf_strneq_n(const char* e1, const char* e2, size_t n) {
  RKT_AssertResPkg* res;
  if (!n) {
...
static inline void RKT_print_test_reg(const RKT_TestEntry* restrict e) {
  ...
  free(fmtbuf);
  for (RK_OutPutStreams i = RK_OUTPUTSTREAMS_STDOUT;
       i <= RK_OUTPUTSTREAMS_STDERR; ++i) {
...
static inline void RKT_print_suite_reg(const RKT_Suite* s, size_t total,
...
static inline void RKT_json_escape(const char* s) {
...
static inline void RKT_json_escape_n(RKT_Strv sv) {
  FILE* out = RKT_glob.output_types[RK_OUTPUTFORMAT_JSON];
  for (size_t i = 0, len = sv.len; i < len; i++) {
    fflush(stdout);
    switch (sv.str[i]) {
    case '\"': fputs("\\\"", out); break;
    case '\\': fputs("\\\\", out); break;
    ...
    case '\t': fputs("\\t", out); break;
    default:
      if ((unsigned char)sv.str[i] < 0x20) {
        fprintf(out, "\\u%04x", (unsigned char)sv.str[i]);
      } else {
        fputc(sv.str[i], out);
      }
    }
...
static inline void RKT_print_test_json(const RKT_TestEntry* e) {
  RKT_TestResult r   = e->res;
  FILE*          out = RKT_glob.output_types[RK_OUTPUTFORMAT_JSON];
  if (out) {
    fputs("    {\n", out);
    fputs("      \"test_name\": \"", out);
    RKT_json_escape_n(e->name), fputs("\",\n", out);
    fputs("      \"file\": \"", out);
    RKT_json_escape_n(e->file), fputs("\",\n", out);
    fputs("      \"tags\": \"", out);
    RKT_json_escape(e->attrs.tags ? e->attrs.tags : ""), fputs("\",\n", out);
    fputs("      \"result\": {\n", out);
    fprintf(out, "        \"ended\": %d,\n", r.ended);
    ...
    if (!r.count) {
      ...
      for (size_t i = 0; i < r.count; i++) {
        ...
        fputs("          {\n", out);
        fputs("            \"expr\": \"", out);
        RKT_json_escape(ad->hdr.exprstr);
        fputs("\",\n", out);
        fprintf(out, "            \"pos\": %d,\n", ad->hdr.pos);
        ...
        for (int j = 0; j < 3; ++j) { // shit
          ...
          fputs("\"", out);
          RKT_Strv arg
              = {ad->res.args[j], ad->res.args[j + 1] - ad->res.args[j] - 1};
          RKT_json_escape_n(arg); // todo find a way to de-stringify
          fputs("\"", out);
        }
        fputs("]\n", out);
        fputs("          }", out);
    ...
    if ((v != RK_VERBOSITY_NEVER)
        && ((v == RK_VERBOSITY_ALWAYS) || r.ended != RKT_PASSED)) {
      fputs(",\n        \"stdout\": \"", out);
      RKT_json_escape_n(e->res.capt[RK_OUTPUTSTREAMS_STDOUT]);
      fputs("\"", out);
    }
    ...
    if ((v != RK_VERBOSITY_NEVER)
        && ((v == RK_VERBOSITY_ALWAYS) || r.ended != RKT_PASSED)) {
      fputs(",\n        \"stderr\": \"", out);
      RKT_json_escape_n(e->res.capt[RK_OUTPUTSTREAMS_STDERR]);
      fputs("\"", out);
    }
    fputs("\n", out);
    fputs("      }\n", out);
...
static inline void RKT_print_suite_json(const RKT_Suite* s, size_t total,
                                        size_t failed, size_t crashed,
                                        size_t timed_out, size_t error) {
  FILE* out = RKT_glob.output_types[RK_OUTPUTFORMAT_JSON];
  if (!out) { return; }
...
static inline void RKT_cleanup_test_res(RKT_TestEntry* e) {
...
static inline void RKT_cleanup_suite_res(const RKT_Suite* s) {
    ...
    *lastres                     = (RKT_AssertRes){RKT_ZINIT};
    ...
    if (WIFSIGNALED(r.term_code)) {
      ...
      if (lasthdr->F == RKTF_CRASH) {
        if (!(lastres->args[0] = (char*)malloc(32))) {
          RKT_fatal(-1, "calloc");
        }
...
static inline void RKT_Windows_Childentry(void) {
  char buf[256];
...
static inline RKT_TestResult RKT_parent_loop(rk_pid pid, rk_fd* pipes,
  ...
  for (int nfds = 3; nfds > 0;) {
    ...
    if (wait == WAIT_FAILED) {
      ...
      TerminateProcess(pid, 1), RKT_reapchild(pid);
    ...
    if (handles[idx] == pipes[RK_OUTPUTSTREAMS_META]) {
      ...
      if (!ReadFile(handles[idx], r.capt[i].str + r.capt[i].len,
        ...
        if (r.capt[i].len == caps[i]) {
          ...
          if (!r.capt[i].str) { RKT_fatal(pid, "realloc stdout/stderr"); }
        }
      }
    }
  ...
  if (reading_res) {
    ...
    if (lasthdr->F == RKTF_CRASH || lasthdr->F == RKTF_EXIT) {
      if (!(lastres->args[0] = (char*)malloc(32))) { RKT_fatal(pid, "malloc"); }
...
#define RKT_REGISTER_TEST_IMPL(SUITENAME, fn, ...)                             \
  ...
  RKT_CONSTRUCTOR(RK_test_register_##fn) {                                     \
    ...
    static RKT_Suite suite = {RKT_ZINIT};                                      \
