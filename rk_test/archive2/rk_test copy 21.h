...
// clang-format off
#define rk_expect_true(expr)                  RKT_AE_true(expect, expr)
#define rk_expect_false(expr)                 RKT_AE_false(expect, expr)
#define rk_expect_null(ptr)                   RKT_AE_null(expect, ptr)
#define rk_expect_nonnull(ptr)                RKT_AE_nonnull(expect, ptr)
#define rk_expect_memeq(ptr1, ptr2, siz)      RKT_AE_memeq(expect, ptr1, ptr2, siz)
#define rk_expect_memneq(ptr1, ptr2, siz)     RKT_AE_memneq(expect, ptr1, ptr2, siz)
#define rk_expect_memzero(ptr, siz)           RKT_AE_memzero(expect, ptr, siz)
#define rk_expect_memnzero(ptr, siz)          RKT_AE_memnzero(expect, ptr, siz)
#define rk_expect_eq(exp, act)                RKT_AE_eq(expect, exp, act)
#define rk_expect_neq(exp, act)               RKT_AE_neq(expect, exp, act)
#define rk_expect_lt(exp, act)                RKT_AE_lt(expect, exp, act)
#define rk_expect_leq(exp, act)               RKT_AE_leq(expect, exp, act)
#define rk_expect_gt(exp, act)                RKT_AE_gt(expect, exp, act)
#define rk_expect_geq(exp, act)               RKT_AE_geq(expect, exp, act)
#define rk_expect_inrange(val, low, high)     RKT_AE_inrange(expect, val, low, high)
#define rk_expect_floateq_tol(exp, act, tol)  RKT_AE_floateq_tol(expect, exp, act, tol)
#define rk_expect_floatneq_tol(exp, act, tol) RKT_AE_floatneq_tol(expect, exp, act, tol)
#define rk_expect_streq(str1, str2)           RKT_AE_streq(expect, str1, str2)
#define rk_expect_strneq(str1, str2)          RKT_AE_strneq(expect, str1, str2)
#define rk_expect_streq_n(str1, str2, len)    RKT_AE_streq_n(expect, str1, str2, len)
#define rk_expect_strneq_n(str1, str2, len)   RKT_AE_strneq_n(expect, str1, str2, len)
#define rk_expect_stdouteq(errstr, len)       RKT_AE_stdouteq(expect, errstr, len)
#define rk_expect_stdoutneq(errstr, len)      RKT_AE_stdoutneq(expect, errstr, len)
#define rk_expect_stderreq(errstr, len)       RKT_AE_stderreq(expect, errstr, len)
#define rk_expect_stderrneq(errstr, len)      RKT_AE_stderrneq(expect, errstr, len)

#define rk_assert_true(expr)                  RKT_AE_true(assert, expr)
#define rk_assert_false(expr)                 RKT_AE_false(assert, expr)
#define rk_assert_null(ptr)                   RKT_AE_null(assert, ptr)
#define rk_assert_nonnull(ptr)                RKT_AE_nonnull(assert, ptr)
#define rk_assert_memeq(ptr1, ptr2, siz)      RKT_AE_memeq(assert, ptr1, ptr2, siz)
#define rk_assert_memneq(ptr1, ptr2, siz)     RKT_AE_memneq(assert, ptr1, ptr2, siz)
#define rk_assert_memzero(ptr, siz)           RKT_AE_memzero(assert, ptr, siz)
#define rk_assert_memnzero(ptr, siz)          RKT_AE_memnzero(assert, ptr, siz)
#define rk_assert_eq(exp, act)                RKT_AE_eq(assert, exp, act)
#define rk_assert_neq(exp, act)               RKT_AE_neq(assert, exp, act)
#define rk_assert_lt(exp, act)                RKT_AE_lt(assert, exp, act)
#define rk_assert_leq(exp, act)               RKT_AE_leq(assert, exp, act)
#define rk_assert_gt(exp, act)                RKT_AE_gt(assert, exp, act)
#define rk_assert_geq(exp, act)               RKT_AE_geq(assert, exp, act)
#define rk_assert_inrange(val, low, high)     RKT_AE_inrange(assert, val, low, high)
#define rk_assert_floateq_tol(exp, act, tol)  RKT_AE_floateq_tol(assert, exp, act, tol)
#define rk_assert_floatneq_tol(exp, act, tol) RKT_AE_floatneq_tol(assert, exp, act, tol)
#define rk_assert_streq(str1, str2)           RKT_AE_streq(assert, str1, str2)
#define rk_assert_strneq(str1, str2)          RKT_AE_strneq(assert, str1, str2)
#define rk_assert_streq_n(str1, str2, len)    RKT_AE_streq_n(assert, str1, str2, len)
#define rk_assert_strneq_n(str1, str2, len)   RKT_AE_strneq_n(assert, str1, str2, len)
#define rk_assert_stdouteq(errstr, len)       RKT_AE_stdouteq(assert, errstr, len)
#define rk_assert_stdoutneq(errstr, len)      RKT_AE_stdoutneq(assert, errstr, len)
#define rk_assert_stderreq(errstr, len)       RKT_AE_stderreq(assert, errstr, len)
#define rk_assert_stderrneq(errstr, len)      RKT_AE_stderrneq(assert, errstr, len)
...
#define RKT_AE_nonnull(AE, ptr)                RKT_AE(AE, RKTf_nonnull, RKTF_NONULL, "nonnull(" #ptr ")", ptr)
...
#define RKT_AE_lt(AE, exp, act)                RKT_AE(AE, RKT_SELFUN(lt, exp), RKTF_LT, "lt(" #exp ", " #act ")", exp, act)
#define RKT_AE_leq(AE, exp, act)               RKT_AE(AE, RKT_SELFUN(leq, exp), RKTF_LEQ, "leq(" #exp ", " #act ")", exp, act)
#define RKT_AE_gt(AE, exp, act)                RKT_AE(AE, RKT_SELFUN(gt, exp), RKTF_GT, "gt(" #exp ", " #act ")", exp, act)
#define RKT_AE_geq(AE, exp, act)               RKT_AE(AE, RKT_SELFUN(geq, exp), RKTF_GEQ, "geq(" #exp ", " #act ")", exp, act)
#define RKT_AE_inrange(AE, val, low, high)     RKT_AE(AE, RKT_SELFUN(inrange, val), RKTF_INRANGE, "inrange(" #val ", " #low", " #high ")", val, low, high)
#define RKT_AE_floateq_tol(AE, exp, act, tol)  RKT_AE(AE, RKT_SELFUN_TOL(toleq, exp), RKTF_FLOATEQ_TOL, "floateq_tol(" #exp", " #act ", " #tol ")", exp, act, tol)
#define RKT_AE_floatneq_tol(AE, exp, act, tol) RKT_AE(AE, RKT_SELFUN_TOL(tolneq, exp), RKTF_FLOATNEQ_TOL,  "floatneq_tol(" #exp", " #act ", " #tol ")", exp, act, tol)
...
#define RKT_AE(AE, EXPRSTR, FUNENUM, FUN, ...) RKT_CONCAT(RKT_, AE)(#AE "_" EXPRSTR, FUN, FUNENUM, __VA_ARGS__)
...
#define sass(lit)                                                              \
  ((struct {                                                                   \
    _Static_assert(sizeof(lit) < 255);                                         \
    int x;                                                                     \
  }){0})
#define RKT_expect(EXPRSTR, FUN, FUNENUM, ...)                                 \
  (RKT_SEND_HDR(EXPRSTR, FUNENUM, 0), FUN(__VA_ARGS__))

#define RKT_assert(EXPRSTR, FUN, FUNENUM, ...)                                 \
...
#define RKT_fmtstub(_S, FMT, _E, _HDR)                                         \
  ...
  default:
    ...
    case RKTF_NONULL: s += RK_catlit(s, RK_INRED(" == 'NULL'")); break;
    case RKTF_EQ:
