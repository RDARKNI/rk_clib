...
private:
  static const size_t hdr_size = sizeof(RKT_AssertResPkg); // todo
...
/*
  ...
  static const char* const fmtstrings[] = {
      [RKTF_TRUE]    = "Evaluated to " RK_INRED("'false'") ".\n",
      [RKTF_FALSE]   = "Evaluated to " RK_INRED("'true'") ".\n",
      [RKTF_NULL]    = "Evaluated to " RK_INRED("%s") ".\n",
      [RKTF_NNULL]   = "Evaluated to " RK_INRED("NULL") ".\n",
      [RKTF_EQ]      = RK_INRED("%s != %s") ".\n",
      [RKTF_TOLEQ]   = RK_INRED("%s != %s") " (tol: %s).\n",
      [RKTF_NEQ]     = RK_INRED("%s == %s") ".\n",
      [RKTF_TOLNEQ]  = RK_INRED("%s == %s") " (tol: %s).\n",
      [RKTF_LT]      = RK_INRED("%s%s%s") ".\n",
      [RKTF_LEQ]     = RK_INRED("%s > %s") ".\n",
      [RKTF_GT]      = RK_INRED("%s%s%s") ".\n",
      [RKTF_GEQ]     = RK_INRED("%s < %s") ".\n",
      [RKTF_INRANGE] = RK_INRED("%s%s%s") ".\n",
      [RKTF_MEMEQ]
      = "Memory Regions %s and %s (%s bytes long) " RK_INRED("not equal") ".\n",
      [RKTF_MEMNEQ]
      = "Memory Regions %s and %s (%s bytes long) " RK_INRED("equal") ".\n",
      [RKTF_MEMZERO]
      = "%s (%s bytes long) " RK_INRED("nonzero at byte %s") ".\n",
      [RKTF_MEMNZERO] = "%s (%s bytes long) " RK_INRED("zero at byte %s") ".\n",
      [RKTF_STREQ]
      = "strings %s, %s " RK_INRED("not equal at position %s") ".\n",
      [RKTF_STRNEQ] = "strings %s, %s " RK_INRED("equal") ".\n",
      [RKTF_CRASH]  = "%s",
      [RKTF_EXIT]   = "%s"
...
rkt_fun size_t RKT_fmt_assertres(const RKT_TestEntry* restrict e,
  ...
  case RKT_ASSERTRES_PASS:
    if (RKT_resolve_verbosity(e, RK_OUTPUTSTREAMS_META)
        == RK_VERBOSITY_ALWAYS) {
      s += RK_fmt_stub(s, RKT_FMT_P, e, hdr), *s++ = '\n';
    }
    break;
  case RKT_ASSERTRES_EXIT:
    s += RK_fmt_stub(s, RKT_FMT_E, e, hdr), *s++ = '\n';
    break;
  case RKT_ASSERTRES_CRASH:
    s += RK_fmt_stub(s, RKT_FMT_C, e, hdr), *s++ = '\n';
    break;
  case RKT_ASSERTRES_TIMEOUT:
    if (!hdr->pos) {
      s += RK_catlit(s, RK_INRED("[T] TEST TIMEOUT\n"));
    } else {
      s += RK_fmt_stub(s, RKT_FMT_T, e, hdr), *s++ = '\n';
    }
    break;
  default:
    ...
    s += RK_fmt_stub(s, RKT_FMT_F, e, hdr), *s++ = ' ';
    const char* fmt;
    switch (hdr->F) {
    case RKTF_TRUE   : fmt = "Evaluated to " RK_INRED("'false'") ".\n"; break;
    case RKTF_FALSE  : fmt = "Evaluated to " RK_INRED("'true'") ".\n"; break;
    case RKTF_NULL   : fmt = "Evaluated to " RK_RED "%s" RK_RESET ".\n"; break;
    case RKTF_NNULL  : fmt = "Evaluated to" RK_INRED("NULL") ".\n"; break;
    case RKTF_EQ     : fmt = RK_INRED("%s != %s") ".\n"; break;
    case RKTF_TOLEQ  : fmt = RK_INRED("%s != %s") " (tol: %s).\n"; break;
    case RKTF_NEQ    : fmt = RK_INRED("%s == %s") ".\n"; break;
    case RKTF_TOLNEQ : fmt = RK_INRED("%s == %s") " (tol: %s).\n"; break;
    case RKTF_LT     : fmt = RK_INRED("%s%s%s") ".\n"; break;
    case RKTF_LEQ    : fmt = RK_INRED("%s > %s") ".\n"; break;
    case RKTF_GT     : fmt = RK_INRED("%s%s%s") ".\n"; break;
    case RKTF_GEQ    : fmt = RK_INRED("%s < %s") ".\n"; break;
    case RKTF_INRANGE: fmt = RK_INRED("%s%s%s") ".\n"; break;
    case RKTF_MEMEQ:
      fmt = "Memory Regions %s and %s (%s bytes long) " RK_INRED(
          "not equal") ".\n";
      break;
    case RKTF_MEMNEQ:
      fmt = "Memory Regions %s and %s (%s bytes long) " RK_INRED("equal") ".\n";
      break;
    case RKTF_MEMZERO:
      fmt = "%s (%s bytes long) " RK_INRED("nonzero at byte %zu") ".\n";
      break;
    case RKTF_MEMNZERO:
      fmt = "%s (%s bytes long) " RK_INRED("zero at byte %zu") ".\n";
      break;
    case RKTF_STREQ:
      fmt = "strings %s, %s " RK_INRED("not equal at position %zu") ".\n";
      break;
    case RKTF_STRNEQ: fmt = "strings %s, %s " RK_INRED("equal") ".\n"; break;
    case RKTF_CRASH:
      if (res.res == 1) {
        fmt = "Did not crash\n";
      } else {
        fmt = "Expected: termination via signal %d, got: "
              "termination with signal %s\n";
      }
      break; // todo signal interpretation
    case RKTF_EXIT:
      if (res.res == 1) {
        fmt = "Did not terminate\n";
      } else {
        fmt = "Expected: exit via code %d, got: "
              "exit with code %s\n";
      }
      break;
    ...
    s += sprintf(fmt, args[0], args[1], args[2], args[3]);
  }
  return s - *fmtbuf;
}
*/
...
rkt_fun size_t RK_fmt_stub(char* restrict s, char* restrict passtype,
                           const RKT_TestEntry* restrict e,
                           const RKT_AssertHdr* hdr) {
  return sprintf(s, "%s %s:%d %s", passtype, e->file.str, hdr->pos,
                 hdr->expr.str);
}
rkt_fun size_t RKT_fmt_assertres(const RKT_TestEntry* restrict e,
                                 RKT_AssertDat cur, char** restrict fmtbuf,
  ...
  default:
    ...
    case RKTF_FALSE:
      ...
      break;
    case RKTF_NULL:
      s += sprintf(s, "Evaluated to " RK_RED "%s" RK_RESET ".\n", args[0]);
      break;
    case RKTF_NNULL:
      s += RK_catlit(s, "Evaluated to" RK_INRED("NULL") ".\n");
      break;
    case RKTF_EQ:
      s += sprintf(s, RK_INRED("%s != %s") ".\n", args[0], args[1]);
      break;
    case RKTF_TOLEQ:
      s += sprintf(s, RK_INRED("%s != %s") " (tol: %s).\n", args[0], args[1],
                   args[2]);
      break;
    case RKTF_NEQ:
      s += sprintf(s, RK_INRED("%s == %s") ".\n", args[0], args[1]);
    ...
    case RKTF_STREQ:
      ...
      break;
    case RKTF_STRNEQ:
      s += sprintf(s, "strings %s, %s " RK_INRED("equal") ".\n", args[0],
                   args[1]);
      break;
    case RKTF_CRASH:
      if (res.res == 1) {
...
rkt_fun RKT_TestEndType RKT_run_test(RKT_TestEntry* e) {
  ...
  return RKT_parent_loop(e, pid, pipes);
}
  ...
  for (int nfds = 3; nfds > 0;) {
    for (int poll_timeout = -1;;) {
      ...
      case 0: {
        ...
        ++r.fails, r.ended = RKT_TIMEDOUT;
        return e->res = r, e->res.ended;
      }
      }
      break;
    }
