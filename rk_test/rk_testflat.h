...
static const bool RKT_print_passes = 0;
#ifdef __GNUC__
...
# define RKT_TMP(type, expr)                                                   \
   (type) { expr }
#endif
#define RKT_ZTMP(type) RKT_TMP(type, RKT_ZINIT)
...
rkt_fun void RKT_send_hdr(int line, RKT_TestAssertFunction f, int sig,
  ...
  memcpy(&hdr.expr.str, str, hdr.expr.len);
  hdr.expr.str[hdr.expr.len] = '\0';
...
#define RKT_SNDBUF0(RES)                                                       \
  ...
  return RKT_send_res(&RKT_glob_buf.pkg);
...
# endif
  ...
  private:
    ...
    static const size_t hdr_size = sizeof(RKT_AssertResPkg),
                        mem_size = sizeof(RKT_glob_buf.storage);
...
rkt_fun size_t RKT_strn_compare_impl_exact(bool want_equal, const char* e1,
                                           ...
                                           size_t l2) {
  RKT_AssertResPkg* res = &RKT_glob_buf.pkg;
  if (!l1 && !l2) { // Treat empty/empty as equal (dif == 0).
    res->res = want_equal ? 0 : 1, res->len = 0;
    return RKT_send_res(res);
  }
  size_t dif = RKT_strdiff(e1, l1, e2, l2);
  size_t r   = want_equal ? dif : !(dif != 0);
  if (RKT_print_passes || r) {
    size_t package_size = sizeof(*res) + l1 + l2 + 2;
    if (package_size > sizeof(RKT_glob_buf.storage)) {
      res = (RKT_AssertResPkg*)malloc(package_size);
      if (!res) { exit(12); }
    }
    res->res = r, res->len = l1 + l2 + 2;
    if (l1) { memcpy(res->args, e1, l1); }
    res->args[l1] = '\0';
    if (l2) { memcpy(res->args + l1 + 1, e2, l2); }
    res->args[l1 + 1 + l2] = '\0';
    RKT_send_res(res);
    if ((void*)res != (void*)&RKT_glob_buf.pkg) { free(res); }
  } else {
    res->res = r, res->len = 0;
    RKT_send_res(res);
  }
...
rkt_fun size_t RKTf_streq_impl(bool eq, const char* e1, const char* e2) {
  // if ((!e1 || !e2) && e1 != e2) {
  // todo
  //   // treat NULL as a printable token and fail appropriately
  // }
  return RKT_strn_compare_impl_exact(eq, e1, strlen(e1), e2, strlen(e2));
}

rkt_fun size_t RKTf_streq_n_impl(bool eq, const char* e1, const char* e2,
                                 size_t n) {
  return RKT_strn_compare_impl_exact(eq, e1, n, e2, n);
}

rkt_fun size_t RKTf_streameq(RK_OutPutStreams stream, bool eq, const char* str,
                             size_t len) {
  fflush(NULL);
  RKT_Strv o   = RKT_readfile(RKT_glob.tmpfds[stream]);
  size_t   res = RKT_strn_compare_impl_exact(eq, str, len, o.str, o.len);
  free(o.str);
  return res;
}
rkt_fun size_t RKTf_streq(const char* e1, const char* e2) {
...
*/
rkt_fun size_t RKT_unpack_args(char* RKT_restrict args_in, size_t len,
...
#define RKT_printstub(_S, _FMT, _E, _HDR, _SUFF)                               \
  ...
  case RKTF_null       : f = RKT_INRED("%s != 'NULL'\n"); break;
  case RKTF_nonnull    : f = RKT_INRED(" == 'NULL'\n"); break;
  case RKTF_eq         :
...
rkt_fun void RKT_cleanup_test_res(RKT_TestEntry* e) {
  RKT_TestResult* r = &e->res;
  for (size_t i = 0; i < r->count; ++i) { free(r->results[i].res.args); }
...
rkt_fun int RKT_write_full(rkt_fd fd, const void* buf, size_t len) {
  ...
  for (DWORD w = 0; len > 0; ptr += (size_t)w, len -= (size_t)w) {
    ...
    if (!WriteFile(fd, ptr, chunk, &w, NULL)) { return -1; }
    if (w == 0) { return -1; }
  }
#endif
  return 1;
...
rkt_fun int RKT_read_res(RKT_AssertRes* RKT_restrict r) {
  int res = RKT_read_full(RKT_glob.log, r, sizeof(RKT_AssertResPkg));
  if (res == 1) {
...
rkt_fun int RKT_read_res_tm(RKT_AssertRes* RKT_restrict r, long ms) {
  int res = RKT_read_full_tm(RKT_glob.log, r, sizeof(RKT_AssertResPkg), ms);
  if (res == 1) {
    if (!r->len) { return r->args = NULL, res; }
    if (!(r->args = RKT_malloc(char, r->len))) { RKT_fatal("malloc read res"); }
...
rkt_fun RKT_TestEndType RKT_finalise_test_noisolation(RKT_TestResult* r,
  ...
  if (exit_condition >= 2) { // timeout, exit status irrelevant
    if (exit_condition < 4) { *hdr = RKT_ZTMP(RKT_AssertHdr); }
    *res     = RKT_ZTMP(RKT_AssertRes);
...
rkt_fun void RKT_parent_loop_noisolation(RKT_TestEntry* RKT_restrict e,
  ...
  for (bool reading_res = 0; cond == 0; reading_res = !reading_res) {
    if (!reading_res) {
      ...
      case 1:
        if (ad->res.res > 0) { ++r->fails, r->ended = RKT_FAILED; };
      }
    }
  }
...
rkt_fun void RKT_finalise_test_isolation(RKT_TestResult* r,
  ...
  *end                  = RKT_PASSED;
  if (exit_condition == 1 || exit_condition == 2) { // timeout, no exit status
    ...
    if (exit_condition == 1) { *hdr = RKT_ZTMP(RKT_AssertHdr); }
    *res     = RKT_ZTMP(RKT_AssertRes);
...
rkt_fun void RKT_parent_loop_isolation(RKT_TestEntry* RKT_restrict e) {
  ...
  for (bool reading_res = 0; cond == -1; reading_res = !reading_res) {
    ...
    if (!reading_res) {
      ...
      case -3:
        r->ended = RKT_TIMEDOUT, ++r->count, cond = 1;
        break;                                     /*timeout no header read*/
      ...
      case -3:
        r->ended = RKT_TIMEDOUT, cond = 2;
        break;                                     /*timeout, header read*/
      case -2: RKT_fatal("read hdr partial read"); /*partial read is error*/
      case -1: RKT_fatal("read res io");           /*other io error*/
      case 0 : cond = 3; break;                     /*crash during run*/
      case 1:
        if (ad->res.res) { ++r->fails, r->ended = RKT_FAILED; }
      }
    }
  }
...
rkt_fun void RKT_init_test(const RKT_TestEntry* RKT_restrict e) {
  ...
  *s = '\0'; /* ensure double-null terminator */
  ...
  BOOL worked   = CreateProcessA(path, cmdline, NULL, NULL, TRUE, 0, nenv, NULL,
                                 &si, &RKT_glob.pid);
