...
#define RK_REGISTER_TEST(SUITENAME, fn, ...)                                   \
    ...
    RKT_CONSTRUCTOR(RK_test_register_##fn) {                                   \
        ...
        entry.name.cstr            = #fn;                                      \
        entry.name.len             = lenof(#fn);                               \
        entry.file.cstr            = __FILE__;                                 \
        entry.file.len             = lenof(__FILE__);                          \
        entry.attrs                = (RKT_CustomAttrs){__VA_ARGS__};           \
        RKT_Suite* s;                                                          \
        for (s = RKT_glob.suites; s; s = s->next) {                            \
            if (!strcmp(s->name, SUITENAME)) {                                 \
                entry.suite = s;                                               \
...
static inline RKT_TestEndType RKT_run_test(const RKT_TestEntry* e) {
    printf(RK_INYELLOW("Running %s\n") "---------------------------------------"
                                       "-----------\n",
           e->name.cstr); // todo print to meta file
    rk_fd          pipes[3];
    FILE*          files[3];
...
static inline rk_pid RKT_init_test(const RKT_TestEntry* restrict e,
    ...
    s    += RK_catstr(s, e->name.cstr, e->name.len);
    *s++  = '\0';
...
int main(int argc, char** argv) {
    ...
    RK_RUN_TESTS(NULL, 0,
                 .verbosity_levels[RKT_PIPE_STDERR] = RK_TEST_LOG_NEVER);
