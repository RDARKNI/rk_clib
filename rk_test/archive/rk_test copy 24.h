...
} RKT_OutPutType;    // todo rename

typedef enum RKT_OutputTypes {
    RKT_LOG_NORMAL,
    RKT_LOG_JSON,
    RKT_LOG_CSV,
} RKT_OutputTypes;
...
static inline void RKT_print_total_beg_json() {
    if (RKT_glob.output_types[RKT_LOG_JSON]) {
        fputs("{\n  \"suites\": [\n", RKT_glob.output_types[RKT_LOG_JSON]);
    }
}
...
// todo NULL etc
static inline size_t RKTf_streq_n(rk_fd fd, const char* e1, const char* e2,
                                  size_t n) {
    RKT_AssertResPkg* res;
    size_t            dif;
    if (!n) {
    ...
    size_t l1 = strlen(e1), l2 = strlen(e2);
    dif = RK_test_strdiff(e1, l1, e2, l2);
    if (l1 + l2 + 2 <= sizeof(RKT_glob.resbufr)) {
        res = (RKT_AssertResPkg*)RKT_glob.resbufr;
    } else {
        res = (RKT_AssertResPkg*)malloc(sizeof(RKT_AssertResPkg) + l1 + l2 + 2);
...
// todo assert_crash without specifying
static inline size_t RKTf_strneq_n(rk_fd fd, const char* e1, const char* e2,
                                   size_t n) {
    RKT_AssertResPkg* res;
    if (!n) {
        res      = (RKT_AssertResPkg*)RKT_glob.resbufr;
...
static inline void RKT_print_test_reg(const RKT_TestEntry* restrict e) {
    ...
    FILE*          out = RKT_glob.output_types[RKT_LOG_NORMAL];
    if (!out) { return; }
    ...
    free(fmtbuf);
    for (int i = RKT_PIPE_STDOUT; i <= RKT_PIPE_STDERR; ++i) {
        RKT_Verbosity v = RKT_resolve_verbosity(e, (RKT_OutPutType)i);
        if ((v != RK_TEST_LOG_NEVER)
            && ((v == RK_TEST_LOG_ALWAYS) || r.ended != RKT_PASSED)) {
            fputs(i == RKT_PIPE_STDOUT ? "Stdout:\n" : "Stderr:\n", out);
    ...
    case RKT_TESTERROR: // todo windows
        ...
        if (WIFSIGNALED(r.term_code)) {
            ...
            fprintf(out,
                    RK_INRED("Error in Test Function; Exited with code %d\n\n"),
...
static inline void RKT_print_suite_reg(const RKT_Suite* s, size_t total,
    ...
    FILE* out = RKT_glob.output_types[RKT_LOG_NORMAL];
    if (!out) { return; }
    // todo print beginning

    for (RKT_TestEntry* e = s->tests; e; e = e->next) { RKT_print_test_reg(e); }
    fprintf(out,
...
static inline void RKT_json_escape(const char* s) {
    FILE* out = RKT_glob.output_types[RKT_LOG_JSON];
    for (; *s; s++) {
        switch (*s) {
...
static inline void RKT_print_test_json(const RKT_TestEntry* e) {
    ...
    if (out) {
        fputs("    {\n", out);
        fputs("      \"test_name\": \"", out);
        RKT_json_escape(e->name.str), fputs("\",\n", out);
        fputs("      \"file\": \"", out);
        RKT_json_escape(e->file.str), fputs("\",\n", out);
        fputs("      \"tags\": \"", out);
        RKT_json_escape(e->attrs.tags ? e->attrs.tags : ""),
            fputs("\",\n", out);
        fputs("      \"result\": {\n", out);
        fprintf(out, "        \"ended\": %d,\n", r.ended);
        ...
        if (!r.count) {
            ...
            for (size_t i = 0; i < r.count; i++) {
                ...
                fputs("            \"args\": [", out);
                for (int j = 0; j < 3; ++j) {
                    if (!ad->res.args[j]) { break; }
                    if (j > 0) { fputs(", ", out); }
                    fputs("\"", out);
                    // todo wrong, something not null-terminated?
                    RKT_json_escape(ad->res.args[j]), fputs("\"", out);
                }
                fputs("]\n", out);
                fputs("          }", out);
        ...
        if (!e->res.capt[0].len) {
            ...
            fputs("        \"stdout\": \"", out);
            // RKT_json_escape(e->res.capt[0].str);
            fputs("        \",\n", out);
        }
        if (!e->res.capt[1].len) {
            fputs("        \"stderr\": \"\"\n", out);
            ...
            fputs("        \"stderr\": \"", out);
            //    RKT_json_escape(e->res.capt[1].str);
            fputs("        \"\n", out);
        }
        // todo
...
static inline void RKT_run_suite(const RKT_Suite* s) {
    ...
    if (RKT_glob.output_types[RKT_LOG_NORMAL]) {}
...
static inline rk_pid RKT_init_test(const RKT_TestEntry* restrict e,
    ...
    fflush(RKT_glob.output_types[0]); // <-- Add this todo
    fflush(RKT_glob.output_types[1]);
    fflush(RKT_glob.output_types[2]);
            ...
            if (!reading_res) {
                ...
                case 1:
                    if (res.len) {
                        ...
                        for (size_t i = 1, len = 0; len < res.len; ++len) {
                            if (!res.args[0][len]) {
...
# define RKT_CONSTRUCTOR(fn)                                                   \
     static void fn(void);                                                     \
     __declspec(allocate(".CRT$XCU")) static void (*RKT_CONCAT(fn, _ptr))(     \
         void)                                                                 \
         = fn;                                                                 \
     static void fn(void)
...
#define RKT_VALIDATE(FUNNAME, RESULT)                                          \
    RK_REGISTER_TEST("testmeta", test_##FUNNAME) {                             \
        if (RKT_CONCAT(RKT_entry_, FUNNAME).suite->attrs.init) {               \
            RKT_CONCAT(RKT_entry_, FUNNAME).suite->attrs.init();               \
        }                                                                      \
        if (!RKT_CONCAT(RKT_entry_, FUNNAME).res.count) {                      \
            rk_assert_eq(RKT_run_test(&RKT_CONCAT(RKT_entry_, FUNNAME)),       \
                         RESULT);                                              \
            RKT_cleanup_test_res(&RKT_CONCAT(RKT_entry_, FUNNAME));            \
        } else {                                                               \
            rk_assert_eq(RKT_CONCAT(RKT_entry_, FUNNAME).res.ended, RESULT);   \
        }                                                                      \
        if (RKT_CONCAT(RKT_entry_, FUNNAME).suite->attrs.fini) {               \
            RKT_CONCAT(RKT_entry_, FUNNAME).suite->attrs.fini();               \
        }                                                                      \
    }
