...
// clang-format off
#define rk_expect_true(expr)                  RK_EXPECT_B(RK_test_true, RK_TEST_TRUE, "rk_expect_true("#expr")", expr)
#define rk_expect_false(expr)                 RK_EXPECT_B(RK_test_false, RK_TEST_FALSE, "rk_expect_false("#expr")", expr)
#define rk_expect_null(ptr)                   RK_EXPECT_V(RK_test_eqnull, RK_TEST_EQNULL, "rk_expect_null("#ptr")", ptr)
#define rk_expect_nonnull(ptr)                RK_EXPECT_V(RK_test_neqnull, RK_TEST_NEQNULL, "rk_expect_nonnull("#ptr")", ptr)
#define rk_expect_memeq(ptr1, ptr2, siz)      RK_EXPECT_V(RK_test_memeq, RK_TEST_MEMEQ, "rk_expect_memeq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_expect_memneq(ptr1, ptr2, siz)     RK_EXPECT_V(RK_test_memneq, RK_TEST_MEMNEQ, "rk_expect_memneq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_expect_memzero(ptr, siz)           RK_EXPECT_V(RK_test_memzero, RK_TEST_MEMZERO, "rk_expect_memzero(" #ptr ",  " #siz ")", ptr, siz)
#define rk_expect_memnzero(ptr, siz)          RK_EXPECT_V(RK_test_memnzero, RK_TEST_MEMNZERO, "rk_expect_memnzero(" #ptr ",  " #siz ")", ptr, siz)

#define rk_expect_eq(exp, act)                RK_EXPECT_G(RK_test_SEL_TF_FUN(eq, exp), RK_TEST_EQ, "rk_expect_eq("#exp", " #act")", exp, act)
#define rk_expect_neq(exp, act)               RK_EXPECT_G(RK_test_SEL_TF_FUN(neq, exp), RK_TEST_NEQ, "rk_expect_neq("#exp", " #act")", exp, act)
#define rk_expect_lt(exp, act)                RK_EXPECT_G(RK_test_SEL_TF_FUN(eq, exp), RK_TEST_LT, "rk_expect_lt("#exp", " #act")", exp, act)
#define rk_expect_leq(exp, act)               RK_EXPECT_G(RK_test_SEL_TF_FUN(neq, exp), RK_TEST_LEQ, "rk_expect_leq("#exp", " #act")", exp, act)
#define rk_expect_gt(exp, act)                RK_EXPECT_G(RK_test_SEL_TF_FUN(eq, exp), RK_TEST_GT, "rk_expect_gt("#exp", " #act")", exp, act)
#define rk_expect_geq(exp, act)               RK_EXPECT_G(RK_test_SEL_TF_FUN(neq, exp), RK_TEST_GEQ, "rk_expect_geq("#exp", " #act")", exp, act)
#define rk_expect_inrange(val, low, high)     RK_EXPECT_G(RK_test_SEL_TF_FUN(inrange, val), RK_TEST_INRANGE, "rk_expect_inrange("#low", " #val ", " #high ")", val, low, high)
#define rk_expect_floateq_tol(exp, act, tol)  RK_EXPECT_F(RK_test_SEL_TF_FUN_TOL(eq, tol), RK_TEST_FLOATEQ_TOL, "rk_expect_floateq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define rk_expect_floatneq_tol(exp, act, tol) RK_EXPECT_F(RK_test_SEL_TF_FUN_TOL(neq, tol), RK_TEST_FLOATNEQ_TOL,  "rk_expect_floatneq_tol("#exp", " #act ", " #tol ")", exp, act, tol)

#define rk_expect_streq(str1, str2)                                                                                      \
    (RK_test_sendhdr(0, "streq(" #str1 ", " #str2 ")", RK_TEST_STREQ, RK_TEST_T_vp),                                      \
     RK_test_streq(RK_OUT, str1, str2)?(void)0:exit(0))
#define rk_expect_strneq(str1, str2)                                                                                     \
    (RK_test_sendhdr(0, "strneq(" #str1 ", " #str2 ")", RK_TEST_STRNEQ, RK_TEST_T_vp),                                    \
     RK_test_strneq(RK_OUT, str1, str2)?(void)0:exit(0))
...
#define rk_assert_true(expr)                  RK_ASSERT_B(RK_test_true, RK_TEST_TRUE, "rk_assert_true("#expr")", expr)
#define rk_assert_false(expr)                 RK_ASSERT_B(RK_test_false, RK_TEST_FALSE, "rk_assert_false("#expr")", expr)
#define rk_assert_null(ptr)                   RK_ASSERT_V(RK_test_eqnull, RK_TEST_EQNULL, "rk_assert_null("#ptr")", ptr)
#define rk_assert_nonnull(ptr)                RK_ASSERT_V(RK_test_neqnull, RK_TEST_NEQNULL, "rk_assert_nonnull("#ptr")", ptr)
#define rk_assert_memeq(ptr1, ptr2, siz)      RK_ASSERT_V(RK_test_memeq, RK_TEST_MEMEQ, "rk_assert_memeq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_assert_memneq(ptr1, ptr2, siz)     RK_ASSERT_V(RK_test_memneq, RK_TEST_MEMNEQ, "rk_assert_memneq(" #ptr1 ", " #ptr2 ", " #siz ")", ptr1, ptr2, siz)
#define rk_assert_memzero(ptr, siz)           RK_ASSERT_V(RK_test_memzero, RK_TEST_MEMZERO, "rk_assert_memzero(" #ptr ",  " #siz ")", ptr, siz)
#define rk_assert_memnzero(ptr, siz)          RK_ASSERT_V(RK_test_memnzero, RK_TEST_MEMNZERO, "rk_assert_memnzero(" #ptr ",  " #siz ")", ptr, siz)

#define rk_assert_eq(exp, act)                RK_ASSERT_G(RK_test_SEL_TF_FUN(eq, exp), RK_TEST_EQ, "rk_assert_eq("#exp", " #act")", exp, act)
#define rk_assert_neq(exp, act)               RK_ASSERT_G(RK_test_SEL_TF_FUN(neq, exp), RK_TEST_NEQ, "rk_assert_neq("#exp", " #act")", exp, act)
#define rk_assert_lt(exp, act)                RK_ASSERT_G(RK_test_SEL_TF_FUN(eq, exp), RK_TEST_LT, "rk_assert_lt("#exp", " #act")", exp, act)
#define rk_assert_leq(exp, act)               RK_ASSERT_G(RK_test_SEL_TF_FUN(neq, exp), RK_TEST_LEQ, "rk_assert_leq("#exp", " #act")", exp, act)
#define rk_assert_gt(exp, act)                RK_ASSERT_G(RK_test_SEL_TF_FUN(eq, exp), RK_TEST_GT, "rk_assert_gt("#exp", " #act")", exp, act)
#define rk_assert_geq(exp, act)               RK_ASSERT_G(RK_test_SEL_TF_FUN(neq, exp), RK_TEST_GEQ, "rk_assert_geq("#exp", " #act")", exp, act)
#define rk_assert_inrange(val, low, high)     RK_ASSERT_G(RK_test_SEL_TF_FUN(inrange, val), RK_TEST_INRANGE, "rk_assert_inrange("#low", " #val ", " #high ")", val, low, high)
#define rk_assert_floateq_tol(exp, act, tol)  RK_ASSERT_F(RK_test_SEL_TF_FUN_TOL(eq, tol), RK_TEST_FLOATEQ_TOL, "rk_assert_floateq_tol("#exp", " #act ", " #tol ")", exp, act, tol)
#define rk_assert_floatneq_tol(exp, act, tol) RK_ASSERT_F(RK_test_SEL_TF_FUN_TOL(neq, tol), RK_TEST_FLOATNEQ_TOL,  "rk_assert_floatneq_tol("#exp", " #act ", " #tol ")", exp, act, tol)

#define rk_assert_streq(str1, str2)                                                                                      \
    (RK_test_sendhdr(1, "streq(" #str1 ", " #str2 ")", RK_TEST_STREQ, RK_TEST_T_vp),                                      \
     RK_test_streq(RK_OUT, str1, str2))
#define rk_assert_strneq(str1, str2)                                                                                     \
    (RK_test_sendhdr(1, "strneq(" #str1 ", " #str2 ")", RK_TEST_STRNEQ, RK_TEST_T_vp),                                    \
     RK_test_strneq(RK_OUT, str1, str2))
...
#  define rk_assert_crash(signal, ...)                                         \
      do {                                                                     \
          RK_test_sendhdr(1, "assert_crash(" #signal ", " #__VA_ARGS__ ")",    \
                          RK_TEST_DEATH, (enum RK_test_TYPE)signal);           \
          __VA_ARGS__;                                                         \
          rk_write_full(RK_OUT, (RK_TestRes[]){{.res = 1}},                    \
                        sizeof(RK_TestRes));                                   \
          exit(0);                                                             \
      } while (0)
...
#  define rk_assert_exit(code, ...)                                            \
      do {                                                                     \
          RK_test_sendhdr(1, "rk_assert_exit(" #code ", " #__VA_ARGS__ ")",    \
                          RK_TEST_EXIT, (enum RK_test_TYPE)code);              \
          __VA_ARGS__;                                                         \
          rk_write_full(RK_OUT, (RK_TestRes[]){{.res = 1}},                    \
                        sizeof(RK_TestRes));                                   \
          exit(0);                                                             \
...
#  define RK_catstr(dst, src, l) (memcpy(dst, src, l), l)
...
enum RK_test_TYPE { RK_TEST_TYPELIST(RK_TEST_GEN_TYPE_ENUM_EL) };
enum RK_test_FUN {
    RK_TEST_TRUE,
    RK_TEST_FALSE,
    RK_TEST_EQNULL,
...
/// @brief Selects the appropriate test function based on type and function type
...
#  define RK_test_SEL_TF_FUN_TOL(FTYPE, ...)                                   \
      _Generic((__VA_ARGS__)RK_TEST_TYPELIST_FLOAT(RK_test_SEL_TF_FUN_TOL_,    \
                                                   FTYPE))
/// @brief Selects the appropriate type enum based on value type
#  define RK_test_SEL_T_ENUM_(T, N, F, ...) , T : RK_TEST_T_##N
#  define RK_test_SEL_T_ENUM(...)                                              \
      _Generic((__VA_ARGS__)RK_TEST_TYPELIST(RK_test_SEL_T_ENUM_),             \
          default: RK_TEST_T_vp)
...
/// @brief the union to hold different argument types (all fundamental
/// types)
typedef union RK_TypeUnion {
    size_t zu;
    RK_TEST_TYPELIST(RK_TEST_GEN_TYPE_STRUCT_EL)
} RK_TypeUnion;
...
typedef struct {
    ...
    enum RK_test_TYPE T;   ///< The argument types (assert_eq_int vs float etc)
    int               pos; ///< The line of the function
    int               exit_on_fail; ///< Whether exp or assert
    const char*       exprstr;
} RK_TestHdr;
...
typedef struct RK_TestRes {
    ...
    RK_TypeUnion args[3]; ///< The evaluated arguments involved in the test
} RK_TestRes;
...
// clang-format off
#define RK_test_GENFUN2(T, N, name, expr)                                       \
    static inline RK_TestRes RK_test_##name##_##N(T e1, T e2) {                 \
        return (RK_TestRes){.res = (expr),                                      \
                            .args = {{.N = e1}, {.N = e2}}};                    \
    }
#define RK_test_GENFUN3(T, N, name, expr)                                       \
    static inline RK_TestRes RK_test_##name##_##N(T e1, T e2, T e3) {           \
        return (RK_TestRes){.res = (expr),                                      \
                            .args = {{.N = e1}, {.N = e2}, {.N = e3}}};         \
    }
#define RK_test_GENFUNS_alltypes(T, N, FMT, ...)                                     \
    RK_test_GENFUN2(T, N, lt, (e1 < e2 ? 0 : (e1 == e2 ? 1 : 2)))               \
    RK_test_GENFUN2(T, N, leq, !(e1 <= e2))                                     \
    RK_test_GENFUN2(T, N, gt, (e1 > e2 ? 0 : (e1 == e2 ? 1 : 2)))               \
    RK_test_GENFUN2(T, N, geq, !(e1 >= e2))                                     \
    RK_test_GENFUN3(T, N, inrange, (e1 < e2 ? 1 : (e1 > e3 ? 2 : 0)))
#define RK_test_GENFUNS_nofloat(T, N, FMT, ...)                                      \
    RK_test_GENFUN2(T, N, eq, !(e1 == e2))                                      \
    RK_test_GENFUN2(T, N, neq, !(e1 != e2))                       
#define RK_test_GENFUNS_float(T, N, FMT, eps_default, fabs_fn)                  \
    RK_test_GENFUN2(T, N, eq, !(fabs_fn(e1 - e2) <= eps_default))               \
    RK_test_GENFUN2(T, N, neq, !(fabs_fn(e1 - e2) > eps_default))               \
    RK_test_GENFUN3(T, N, eq_tol, !(fabs_fn(e1 - e2) <= e3))                    \
    RK_test_GENFUN3(T, N, neq_tol, !(fabs_fn(e1 - e2) > e3))
    
RK_TEST_TYPELIST_NOFLOAT(RK_test_GENFUNS_nofloat)                
RK_TEST_TYPELIST(RK_test_GENFUNS_alltypes)                       
RK_test_GENFUNS_float(float, f, "%f", 1e-6f, fabsf)              
RK_test_GENFUNS_float(double, d, "%f", 1e-12, fabs)          
RK_test_GENFUNS_float(long double, ld, "%Lf", 1e-12L, fabsl) 

static inline RK_TestRes RK_test_true(bool _res) {
    return (RK_TestRes){.res = !(_res)};
}
static inline RK_TestRes RK_test_false(bool _res) {
    return (RK_TestRes){.res = !(!_res)};
}
static inline RK_TestRes RK_test_eqnull(const void* ptr) {
    return (RK_TestRes){.res = !(ptr == NULL), .args = {{.vp = ptr}}};
}
static inline RK_TestRes RK_test_neqnull(const void* ptr) {
    return (RK_TestRes){.res = !(ptr != NULL), .args = {{.vp = ptr}}};
}
static inline RK_TestRes RK_test_memeq(const void* e1, const void* e2, size_t e3) {
    return (RK_TestRes){.res = !((e1 == e2) || (e1 && e2 && !memcmp(e1, e2, e3))),
                        .args = {{.vp = e1}, {.vp = e2}, {.zu = e3}}};
}
static inline RK_TestRes RK_test_memneq(const void* e1, const void* e2, size_t e3) {
    return (RK_TestRes){.res = !!((e1 == e2) || (e1 && e2 && !memcmp(e1, e2, e3))),
                        .args = {{.vp = e1}, {.vp = e2}, {.zu = e3}}};
}
static inline RK_TestRes RK_test_memzero(const void* e1, size_t size) {
    size_t      dif = 0;
    for (size_t i = 0; i < size; ++i) {
        if (((const char*)e1)[i]) { dif = i + 1; break; }
    }
    return (RK_TestRes){.res = dif, .args = {{.vp = e1}, {.zu = size}}};
}
static inline RK_TestRes RK_test_memnzero(const void* e1, size_t size) {
    const void* loc;
    return (RK_TestRes){.res  = (!size || !(loc = memchr(e1, 0, size)))
                                  ? 0
                                  : ((char*)loc - (char*)e1 + 1),
                        .args = {{.vp = e1}, {.zu = size}}};
}
static inline bool RK_test_streq(rk_fd p, const char* e1, const char* e2) {
    size_t     l1 = e1 ? strlen(e1) : 0, l2 = e2 ? strlen(e2) : 0;
    size_t     dif = RK_test_strdiff(e1, l1, e2, l2);
    RK_TestRes r   = {.res = dif, .args = {{.zu = l1}, {.zu = l2}}};
    rk_write_full(p, &r, sizeof(r));
    if (dif != 0) { // if they're not equal send both strings
        rk_write_full(p, e1, l1), rk_write_full(p, e2, l2);
    }
    return dif == 0;
}
static inline bool RK_test_strneq(rk_fd p, const char* e1, const char* e2) {
    size_t     l1 = e1 ? strlen(e1) : 0, l2 = e2 ? strlen(e2) : 0;
    size_t     dif = RK_test_strdiff(e1, l1, e2, l2);
    RK_TestRes r   = {.res = !(dif != 0), .args = {{.zu = l1}, {.zu = l2}}};
    rk_write_full(p, &r, sizeof(r));
    if (dif == 0) { rk_write_full(p, e1, l1); }
    return dif != 0;
}
...
#define RK_INRED(str)    "\x1b[31m" str "\x1b[0m"
#define RK_INGREEN(str)  "\x1b[32m" str "\x1b[0m"
#define RK_INBLUE(str)   "\x1b[34m" str "\x1b[0m"
#define RK_INYELLOW(str) "\x1b[33m" str "\x1b[0m"
#define RK_INBOLD(str)   "\x1b[1m" str "\x1b[22m"
...
#define RK_TFMT_NULL(res)    "Expected " RK_INGREEN("'NULL'") ", got: " RK_INRED("'%p'") ".",          \
                               res.args[0].vp
#define RK_TFMT_NONNULL(res) "Expected " RK_INGREEN("nonnull pointer") ", got: " RK_INRED("'%p'") ".", \
                               res.args[0].vp
#define RK_TFMT_MEMEQ(res)   "Memory Regions %p and %p (%zu bytes long) " RK_INRED("not equal") ".",   \
                               res.args[0].vp, res.args[1].vp, res.args[2].zu
#define RK_TFMT_MEMNEQ(res)  "Memory Regions %p and %p (%zu bytes long) " RK_INRED("equal") ".",       \
                               res.args[0].vp, res.args[1].vp, res.args[2].zu
#define RK_TFMT_MEMZERO(res) "%p (%zu bytes long) nonzero at byte " RK_INRED("%zu") ".",               \
                               res.args[0].vp, res.args[1].zu, res.res - 1
#define RK_TFMT_MEMNZER(res) "%p (%zu bytes long) zero at byte " RK_INRED("%zu") ".",                  \
                               res.args[0].vp, res.args[1].zu, res.res - 1
#define RK_TFMT_STREQ(res)   "strings (lens: %zu, %zu) " RK_INRED("not equal") " at position %zu.",    \
                               res.args[0].zu, res.args[1].zu, res.res - 1
#define RK_TFMT_STRNEQ(res)  "strings (len: %zu) " RK_INRED("equal") ".",                              \
                               res.args[0].zu
...
#define RK_test_sendhdr(IS_ASSERT, EXPRSTR, FUNENUM, TYPEENUM)                 \
    RK_test_send_hdr(RK_OUT, (RK_TestHdr){.exit_on_fail = IS_ASSERT,           \
                                          .F            = FUNENUM,             \
                                          .T            = TYPEENUM,            \
                                          .pos          = __LINE__,            \
                                          .exprstr      = EXPRSTR})
#define RK_test_sendhdr_generic(IS_ASSERT, EXPRSTR, FUNENUM, ...)              \
    RK_test_sendhdr(IS_ASSERT, EXPRSTR, FUNENUM,                               \
                    RK_test_SEL_T_ENUM(__VA_ARGS__))

#define RK_EXPECT_G(FUN, FUNENUM, EXPRSTR, arg1, ...)                          \
    (RK_test_sendhdr_generic(0, EXPRSTR, FUNENUM, arg1),                       \
     RK_test_send_res_expect(RK_OUT, FUN(arg1, __VA_ARGS__)))
#define RK_EXPECT_B(FUN, FUNENUM, EXPRSTR, expr)                               \
    (RK_test_sendhdr(0, EXPRSTR, FUNENUM, RK_TEST_T_b),                        \
     RK_test_send_res_expect(RK_OUT, FUN(expr)))
#define RK_EXPECT_V(FUN, FUNENUM, EXPRSTR, ...)                                \
    (RK_test_sendhdr(0, EXPRSTR, FUNENUM, RK_TEST_T_vp),                       \
     RK_test_send_res_expect(RK_OUT, FUN(__VA_ARGS__)))
#define RK_EXPECT_F(FUN, FUNENUM, EXPRSTR, arg1, ...)                          \
    (RK_test_sendhdr(0, EXPRSTR, FUNENUM,                                      \
                     _Generic((arg1),                                          \
                         float: RK_TEST_T_f,                                   \
                         double: RK_TEST_T_d,                                  \
                         long double: RK_TEST_T_ld)),                          \
     RK_test_send_res_expect(RK_OUT, FUN(arg1, __VA_ARGS__)))

#define RK_ASSERT_G(FUN, FUNENUM, EXPRSTR, arg1, ...)                          \
    (RK_test_sendhdr_generic(1, EXPRSTR, FUNENUM, arg1),                       \
     RK_test_send_res_assert(RK_OUT, FUN(arg1, __VA_ARGS__)))
#define RK_ASSERT_B(FUN, FUNENUM, EXPRSTR, expr)                               \
    (RK_test_sendhdr(1, EXPRSTR, FUNENUM, RK_TEST_T_b),                        \
     RK_test_send_res_assert(RK_OUT, FUN(expr)))
#define RK_ASSERT_V(FUN, FUNENUM, EXPRSTR, ...)                                \
    (RK_test_sendhdr(1, EXPRSTR, FUNENUM, RK_TEST_T_vp),                       \
     RK_test_send_res_assert(RK_OUT, FUN(__VA_ARGS__)))
#define RK_ASSERT_F(FUN, FUNENUM, EXPRSTR, arg1, ...)                          \
...
// clang-format on
#  define RK_TEST_GEN_T_SWITCHCASE(T, N, FMT, ...)                             \
  case RK_TEST_T_##N: return sprintf(str, FMT, res.N);
static inline int RK_test_catarg(char* str, enum RK_test_TYPE type,
                                 RK_TypeUnion res) {
    switch (type) { RK_TEST_TYPELIST(RK_TEST_GEN_T_SWITCHCASE) }
}

static inline void RK_test_log_streqstrs(FILE* w_file, rk_fd meta_r,
                                         RK_TestRes res, char* buf,
                                         size_t bufsiz, int nstrs) {
    for (int i = 0; i < nstrs; ++i) {
        fputs(" --> " RK_INBLUE("|"), w_file);
        for (size_t toread = res.args[i].zu; toread > 0;) {
            size_t min = RK_MIN(bufsiz, toread);
            switch (rk_read_full(meta_r, buf, min)) {
            case RK_IO_RESULT_ERR: perror("read string"); // fallthrough
            case RK_IO_RESULT_EOF: exit(1);
            default              : fwrite(buf, 1, min, w_file), toread -= min;
            }
        }
        fputs(RK_INBLUE("|") "\n", w_file);
    }
...
static inline size_t RK_test_log_failmsg(RK_TestHdr hdr, RK_TestRes res,
    ...
    case RK_TEST_INRANGE:
        ...
        s += RK_test_catarg(s, hdr.T, res.args[0]);
        switch (hdr.F) {
        case RK_TEST_EQ:
        ...
        case RK_TEST_INRANGE:
            ...
            s += RK_test_catarg(s, hdr.T, res.args[1 + (res.res == 2)]);
            goto afterop2;
        default: unreachable();
        }
        s += RK_test_catarg(s, hdr.T, res.args[1]);
    afterop2:
        s += RK_catlit(s, RK_RESET);
        if (hdr.F == RK_TEST_FLOATEQ_TOL || hdr.F == RK_TEST_FLOATNEQ_TOL) {
            s    += RK_catlit(s, " (tol: ");
            s    += RK_test_catarg(s, hdr.T, res.args[2]);
            *s++  = ')';
        }
        break;
...
static inline int RK_test_end_parent_loop(RK_test_entry* t, RK_TestHdr hdr,
                                          ...
                                          size_t* lens, char* buf, char* s) {
    struct {
        enum { RK__NORMAL, RK__EXIT, RK__SIGN } kind;
        int code;
    } exp, act;
    FILE* fm = w_files[0];
    switch (hdr.F) {
...
#  ifndef _WIN32
    switch ((act.kind = reading_res * (1 + WIFSIGNALED(status)))) {
...
#  else
    ...
    act.code = (int)status;
#  endif
    enum { P, F, C, E } test_state = t->fails > 0;
...
static inline int RK_test_parent_loop(RK_test_entry* t, rk_pid pid,
                                      rk_fd* r_fds, FILE** w_files) {
    printf(RK_TFMT_TEST_RUNSTART(t));
    struct pollfd fds[3] = {{r_fds[0], .events = POLLIN},
                            {r_fds[1], .events = POLLIN},
                            {r_fds[2], .events = POLLIN}};
    enum { INITSIZ = 512, MAXSIZ = 1024 * 1024 * 50 };
    char*  bufs[3] = {[1] = malloc(INITSIZ), [2] = malloc(INITSIZ)};
    size_t lens[3] = {0};
    size_t caps[3] = {[1] = INITSIZ, [2] = INITSIZ};
                ...
                case RK_IO_RESULT_OK:
                    ...
                    if (!res.res) {
                        ...
                        int nstrs = hdr.F == RK_TEST_STREQ  ? 2
                                  : hdr.F == RK_TEST_STRNEQ ? 1
                                                            : 0;
                        RK_test_log_streqstrs(fm, fds[0].fd, res, buf,
                                              sizeof(buf), nstrs);
                    }
                    s = buf + lenof(RK_TFMT_P);
                }
