...
#define RK_REGISTER_SUITE(SUITENAME, ...)                                      \
    RKT_CONSTRUCTOR(RK_test_suite_register_##__LINE__) {                       \
...
typedef struct RKT_Arena {
    char*  data;
    size_t end, cur;
} RKT_Arena;

static inline size_t RKT_arena_alloc(RKT_Arena* restrict arena, size_t size) {
    size = (size + 15) & ~15;
    if (arena->cur + size > arena->end) {
        size_t nsize = arena->end * 2;
        while (nsize - arena->cur < size) { nsize *= 2; }
        if (!(arena->data = realloc(arena->data, nsize))) {
            perror("realloc"), exit(1);
        }
        arena->end = nsize;
    }
    size_t off  = arena->cur;
    arena->cur += size;
    return off;
}
static inline size_t RKT_arena_realloc(RKT_Arena* restrict arena, size_t offset,
                                       size_t oldsize, size_t newsize) {
    if (offset + oldsize != arena->cur) {
        size_t nof = RKT_arena_alloc(arena, newsize);
        memcpy(arena->data + nof, arena->data + offset,
               RK_MIN(oldsize, newsize));
        return nof;
    }
    size_t nsiz = arena->end * 2;
    while (nsiz - arena->cur < newsize) { nsiz *= 2; }
    if (!(arena->data = realloc(arena->data, nsiz))) {
        perror("realloc"), exit(1);
    }
    arena->end = nsiz;
    arena->cur = offset + newsize;
    return offset;
}
#define RKT_ptr(ARENA, TYPE, OFFSET) ((TYPE*)((ARENA)->data + (OFFSET)))
#define RKT_alloc(ARENA, TYPE, SIZE)                                           \
    ((RKT_arena_alloc((ARENA), sizeof(TYPE) * (SIZE))))
#define RKT_realloc(ARENA, TYPE, OFFSET, OLDSIZE, NEWSIZE)                     \
    RKT_arena_realloc((ARENA), (OFFSET), sizeof(TYPE) * (OLDSIZE),             \
                      sizeof(TYPE) * (NEWSIZE))
...
typedef struct RKT_TestResult {
    ...
    size_t results; /// RKT_AssertDat* < Result of each assertion in the test
    struct {
        size_t dat; // char*
        size_t len;
...
static inline rk_pid          RKT_init_test(const RKT_TestEntry* restrict e,
                                            ...
                                            FILE** restrict files);
static inline RKT_TestResult  RKT_parent_loop(RKT_Arena* restrict a, rk_pid pid,
                                              rk_fd* pipes, long timeout_ms);
static inline RKT_TestEndType RKT_print_testres(const RKT_Arena* restrict a,
                                                const RKT_TestEntry* restrict e,
                                                RKT_TestResult t,
                                                FILE** restrict files);
static inline size_t          RKT_print_assertres(const RKT_Arena* restrict a,
                                                  const RKT_TestEntry* restrict e,
                                                  RKT_AssertDat cur,
...
static inline size_t RKTf_streq(rk_fd fd, const char* e1, const char* e2) {
    ...
    if (l1 + l2 + 2 <= sizeof(RKT_glob.resbufr)) {
        ...
        res = (RKT_AssertResPkg*)malloc(sizeof(*res) + l1 + l2 + 2);
        if (!res) { exit(1); }
    }
...
static inline RKT_TestEndType RKT_run_test(const RKT_TestEntry* e) {
    ...
    RKT_Arena      a   = {malloc(2 << 12), 2 << 12, 0};
    // todo pid issue in arena
    RKT_TestResult results
        = RKT_parent_loop(&a, pid, pipes, RKT_resolve_timeout(e));
    RKT_TestEndType res = RKT_print_testres(&a, e, results, files);
    free(a.data);
    for (int i = 0; i < 3; ++i) {
...
static inline size_t RKT_print_assertres(const RKT_Arena* restrict a,
    ...
    char*         args[4] = {0};
    for (int i = 0; i < 4; ++i) { // todo wrong
        if (!res.args[i]) { break; }
        args[i] = RKT_ptr(a, char, res.args[i]);
    }
    ...
    default:
        if (res.len + 256 > *fmtcap) {
            *fmtcap = res.len + 256;
            *fmtbuf = (char*)realloc(*fmtbuf, *fmtcap);
            if (!*fmtbuf) {
...
static inline RKT_TestEndType RKT_print_testres(const RKT_Arena* restrict a,
    ...
    for (size_t i = 0; i < r.count; ++i) {
        size_t bytes = RKT_print_assertres(
            a, e, RKT_ptr(a, RKT_AssertDat, r.results)[i], &fmtbuf, &fmtcap);
        fwrite(fmtbuf, 1, bytes, files[2]);
    }
    free(fmtbuf);
    for (RKT_OutPutType pt = RKT_PIPE_STDOUT; pt <= RKT_PIPE_STDERR; ++pt) {
        RKT_Verbosity v = RKT_resolve_verbosity(e, pt);
        if ((v != RK_TEST_LOG_NEVER)
            && ((v == RK_TEST_LOG_ALWAYS) || r.ended != RKT_PASSED)) {
            fwrite(RKT_ptr(a, char, r.capt[pt].dat), 1, r.capt[pt].len,
                   files[pt]);
        }
    }
    switch (r.ended) {
...
static inline RKT_TestResult RKT_parent_loop(RKT_Arena* restrict a, rk_pid pid,
    ...
    RKT_TestResult r       = {
              .count   = 0,
              .fails   = 0,
              .results = RKT_alloc(a, RKT_AssertDat, rescap),
              .capt = {{RKT_alloc(a, char, caps[1])}, {RKT_alloc(a, char, caps[1])}}};
    ...
    for (int nfds = 3; nfds > 0;) {
        for (int poll_timeout = -1;;) {
            ...
            if (p == -1) {
                ...
                if (reading_res) {
                    RKT_ptr(a, RKT_AssertDat, r.results)[r.count - 1].res = res;
                    ((RKT_AssertDat*)(a->data + r.results))->res          = res;
                } else {
                    RKT_AssertDat dat = {{RKT_ZINIT}, res};
                    RKT_ptr(a, RKT_AssertDat, r.results)[r.count++] = dat;
                }
        ...
        if (fds[2].revents & POLLIN) {
            if (!reading_res) {
                if (r.count + 1 == rescap) { //+1 to simplify timeout code
                    r.results = RKT_realloc(a, RKT_AssertDat, r.results, rescap,
                                            rescap * 2);
                    rescap *= 2;
                }
                ...
                case 1:
                    RKT_ptr(a, RKT_AssertDat, r.results)[r.count++].hdr = hdr;
                    reading_res                                         = 1;
                }
            } else {
                ...
                case 1:
                    if (res.len) {
                        res.args[0] = RKT_alloc(a, char, res.len);
                        char* start = RKT_ptr(a, char, res.args[0]);
                        res.args[1] = 0, res.args[2] = 0, res.args[3] = 0;
                        // printf("WATCH:%s\n", start);
                        if (RKT_read_full_nb(fds[2].fd, start, res.len) < 1) {
        ...
        for (int i = 0; i < 2; ++i) { // todo maybe drain pipe?
            ...
            ssize_t rd = read(fds[i].fd,
                              RKT_ptr(a, char, r.capt[i].dat) + r.capt[i].len,
                              caps[i] - r.capt[i].len);
            if (rd == -1) {
                ...
                r.capt[i].dat
                    = RKT_realloc(a, char, r.capt[i].dat, caps[i], caps[i] * 2);
                caps[i] *= 2;
            }
        }
    }
    ...
    if (reading_res) {
        RKT_AssertRes* lastres
            = &(RKT_ptr(a, RKT_AssertDat, r.results)[r.count - 1].res);
        *lastres = (RKT_AssertRes){RKT_ZINIT};
        const RKT_AssertHdr* lasthdr
            = &(RKT_ptr(a, RKT_AssertDat, r.results)[r.count - 1].hdr);
        if (WIFSIGNALED(r.term_code)) {
            int sig = WTERMSIG(r.term_code);
            if (lasthdr->F == RKTF_CRASH) {
                char* firstarg = RKT_ptr(a, char, lastres->args[0]);
                lastres->len   = sprintf(firstarg, "%d", sig) + 1;
            ...
            if (lasthdr->F == RKTF_EXIT) {
                char* firstarg = RKT_ptr(a, char, lastres->args[0]);
                lastres->len   = sprintf(firstarg, "%d", stat) + 1;
...
static inline RKT_TestResult RKT_parent_loop(rk_pid pid, rk_fd* pipes,
    ...
    r.capt[0]   = {RKT_malloc(char, caps[0]), 0};
    r.capt[1]   = {RKT_malloc(char, caps[1]), 0};
    r.term_code = 0;
    r.ended     = RKT_PASSED;
