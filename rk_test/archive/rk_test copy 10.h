...
typedef struct RKT_AssertHdr {
    const char*      exprstr; ///< TODO IMPORTANT NOT PORTABLE
...
static inline size_t RKT_print_assertres(const RKT_TestEntry* restrict e,
    ...
    *s++  = ' ';
    switch (hdr.F) {
    case RKTF_TRUE   : s += RK_catlit(s, "Expected " RK_INGREEN("'true'") ", got: " RK_INRED("'false'") "."); break;
    case RKTF_FALSE  : s += RK_catlit(s, "Expected " RK_INGREEN("'false'") ", got: " RK_INRED("'true'") "."); break;
    case RKTF_NULL : s += sprintf(s, "Expected " RK_INGREEN("'NULL'") ", got: " RK_INRED("'%s'") ".", res.args[0]); break;
    case RKTF_NNULL: s += sprintf(s, "Expected " RK_INGREEN("nonnull pointer") ", got: " RK_INRED("'%s'") ".", res.args[1]); break;
    case RKTF_EQ:
    case RKTF_TOLEQ:
    ...
    case RKTF_LEQ:
        s += sprintf(s, "%s > %s", res.args[0], res.args[1]);
        break;
    case RKTF_GT      : s += sprintf(s, "%s %s %s", res.args[0], res.res == 1 ? "==" : "<", res.args[1]); break;
    case RKTF_GEQ     : s += sprintf(s, "%s < %s", res.args[0], res.args[1]); break;
    case RKTF_INRANGE : s += sprintf(s, "%s %s %s", res.args[0], res.res == 1 ? "<" : ">", res.args[1 + (res.res == 2)]); break;
    case RKTF_MEMEQ   : s += sprintf(s, "Memory Regions %s and %s (%s bytes long) " RK_INRED("not equal") ".",   \
                               res.args[0], res.args[1], res.args[2]); break;
    case RKTF_MEMNEQ  : s += sprintf(s, "Memory Regions %s and %s (%s bytes long) " RK_INRED("equal") ".",       \
                               res.args[0], res.args[1], res.args[2]); break;
    case RKTF_MEMZERO : s += sprintf(s, "%s (%s bytes long) nonzero at byte " RK_INRED("%zu") ".",               \
                               res.args[0], res.args[1], res.res - 1); break;
    case RKTF_MEMNZERO: s += sprintf(s, "%s (%s bytes long) zero at byte " RK_INRED("%zu") ".",                  \
                               res.args[0], res.args[1], res.res - 1); break;
    case RKTF_STREQ   : s += sprintf(s, "strings %s, %s " RK_INRED("not equal") " at position %zu.",    \
                               res.args[0], res.args[1], res.res - 1); break;
    case RKTF_STRNEQ  : s += sprintf(s, "strings " RK_INRED("equal") "."); break;
    case RKTF_CRASH:
        if (res.res == 1) {
            s += sprintf(s, " - Expected: termination via signal %d, got: no termination",            \
                               (int)(hdr.code));
        } else {
            s += sprintf(s,
...
__declspec(noreturn) static inline void RKT_fatal(rk_pid pid, const char* msg) {
