...
#  define rk_assert_crash(signal, ...)                                         \
      do {                                                                     \
          RK_test_sendhdr_sig(1,                                               \
                              "assert_crash(" #signal ", " #__VA_ARGS__ ")",   \
...
#  define rk_assert_exit(code, ...)                                            \
      do {                                                                     \
          RK_test_sendhdr_sig(1,                                               \
                              "rk_assert_exit(" #code ", " #__VA_ARGS__ ")",   \
                              RK_TEST_EXIT, code);                             \
          __VA_ARGS__;                                                         \
          rk_write_full(RK_OUT, (RK_TestRes[]){{.res = 1}},                    \
                        sizeof(RK_TestRes));                                   \
...
#   endif
#  endif
#  define RK_lenof(strlit) (sizeof("" strlit "") - 1)
#  define RK_catlit(s, l)  (memcpy(s, l, RK_lenof(l)), RK_lenof(l))
...
struct RK_streamable : std::false_type {};

template <typename T>
struct RK_streamable<
...
#   endif
        if constexpr (RK_streamable<T>::value) {
...
RK_test_GENFUN2_sig(T, N, FMT, name)                                   {                 \
    RK_TestRes res;\
    res.res = (expr);\
    sprintf(res.objs[0],FMT,e1), sprintf(res.objs[1],FMT,e2);\
    return res;\
}
#define RK_test_GENFUN3(T, N, FMT, name, expr)                                       \
 RK_test_GENFUN3_sig(T, N, FMT, name) {           \
    RK_TestRes res;\
    res.res = (expr);\
    sprintf(res.objs[0],FMT,e1);\
    sprintf(res.objs[1],FMT,e2);\
    sprintf(res.objs[2],FMT,e3);\
    return res;\
}
...
static inline RK_TestRes RK_test_memeq(const void* e1, const void* e2, size_t e3) {
    ...
    sprintf(res.objs[0],"%p", e1);
    sprintf(res.objs[1],"%p", e2);
    sprintf(res.objs[2],"%zu", e3);
    return res;
}
...
static inline RK_TestRes RK_test_memzero(const void* e1, size_t e2) {
    ...
    RK_TestRes res;
    res.res = dif;
    sprintf(res.objs[0],"%p", e1), sprintf(res.objs[1],"%zu", e2);
    return res;
}
...
static inline RK_TestRes RK_test_streq(const char* e1, const char* e2) {
    ...
    RK_TestRes res;
    res.res = dif;
    snprintf(res.objs[0],sizeof(res.objs[0]),"%s", e1);
    snprintf(res.objs[1],sizeof(res.objs[1]),"%s", e2);
    return res;
}
static inline RK_TestRes RK_test_strneq(const char* e1, const char* e2) {
    size_t     l1 = e1 ? strlen(e1) : 0, l2 = e2 ? strlen(e2) : 0;
    size_t     dif = RK_test_strdiff(e1, l1, e2, l2);
    RK_TestRes res   = {.res = !(dif != 0)};
    snprintf(res.objs[0],sizeof(res.objs[0]),"%s", e1);
    snprintf(res.objs[1],sizeof(res.objs[1]),"%s", e2);
    return res;
}
...
static inline int RK_test_parent_loop(RK_test_entry* t, rk_pid pid,
    ...
    RK_TestHdr hdr = {};
    RK_TestRes res = {0};
    enum { RK_PREFLEN = RK_lenof(RK_TFMT_P) };
    FILE* fm = w_files[0];
...
static inline int RK_test_run_test(RK_test_entry* t) {
    ...
    { // spawn the child
        ...
        size_t      extra = RK_lenof("RK_CHILD_FN=") + strlen(t->name) + 1
                     + RK_lenof("RK_CHILD_META=") + 32 + 1 + 1;
