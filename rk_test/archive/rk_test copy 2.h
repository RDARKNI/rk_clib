...
NORETURN static inline void RKT_fatal(pid_t pid, const char* str) {
    perror(str);
    if (pid > 0) {
        kill(pid, SIGKILL);
...
static inline void RKT_setup_files(RK_test_entry* t, FILE** files, rk_pid pid) {
    for (int i = 0; i < 3; ++i) {
        if (!t->attrs._[i].path) {
...
static inline rk_pid RK_test_spawnchild(RK_test_entry* t, rk_fd* pipes,
                                        FILE** files) {
...
static inline int RK_test_run_test(RK_test_entry* t) {
    ...
    rk_fd pipes[3];
    FILE* files[3];
    // pid_t pid = RK_test_spawnchild(t, pipes, files);
    pid_t pid = RK_fork_unipipe_n(pipes, UNIPIPE_DIR_CTOP, 3);
    switch (pid) {
    case -2: RKT_fatal(pid, "fork");
    case -1: RKT_fatal(pid, "pipe");
    ...
    default:
        ...
        return RK_test_parent_loop(t, pid, pipes, files);
    }
}
...
static inline int RK_test_parent_loop(RK_test_entry* t, rk_pid pid,
    ...
    for (int nfds = 3; nfds > 0;) {
        ...
        if (wait == WAIT_FAILED) { RKT_fatal(pid, "WaitForMultipleObjects"); }
        ...
        if (handles[idx] == r_fds[0]) {
            ...
            if (!ReadFile(r_fds[i], bufs[i] + lens[i],
                ...
                if (lens[i] == caps[i]) {
                    caps[i] *= 2, bufs[i] = (char*)realloc(bufs[i], caps[i]);
                    if (!bufs[i]) { RKT_fatal(pid, "realloc stdout/stderr"); }
                }
            }
        }
