...
static inline int  RK_test_parent_loop(RK_test_entry* t, rk_fd* r_fds,
                                       FILE** w_files);
...
NORETURN static inline void RKT_fatal(RK_test_entry* t, const char* str) {
    perror(str);
    if (t->pid > 0) {
        kill(t->pid, SIGKILL);
...
static inline void RKT_setup_files(RK_test_entry* t, FILE** files) {
    for (int i = 0; i < 3; ++i) {
        if (!t->attrs._[i].path) {
            files[i] = RK_test_glob.files[i];
        } else if (!(files[i] = fopen(t->attrs._[i].path, "w"))) {
            RKT_fatal(t, "fopen");
        }
    }
}

static inline void RK_test_close_files(RK_test_entry* t, FILE** files,
                                       rk_pid pid) {
    for (int i = 0; i < 3; ++i) {
        if (t->attrs._[i].path) { fclose(files[i]); } // todo
    }
}
static inline int RK_test_run_test(RK_test_entry* t) {
    int   res;
    rk_fd pipes[3];
    FILE* files[3];
    switch (t->pid = RK_fork_unipipe_n(pipes, UNIPIPE_DIR_CTOP, 3)) {
    case -2: RKT_fatal(t, "fork");
    case -1: RKT_fatal(t, "pipe");
    case 0:
    ...
    default:
        ...
        return RK_test_parent_loop(t, pipes, files);
    }
}
static inline int RKT_reapchild(RK_test_entry* t) {
    int status;
...
static inline int RK_test_parent_loop(RK_test_entry* t, rk_fd* r_fds,
    ...
    return RK_test_end_parent_loop(t, hdr, w_files, RKT_reapchild(t),
...
static inline int RK_test_parent_loop(RK_test_entry* t, rk_fd* r_fds,
    ...
    FILE*      fm          = w_files[0];
    int        vb          = RK_test_glob.attrs.meta.verbosity;
    rk_fd      handles[4]  = {r_fds[0], r_fds[1], r_fds[2]};
    bool       reading_res = 0;
    for (int nfds = 3; nfds > 0;) {
        ...
        if (handles[idx] == r_fds[0]) {
            ...
            size_t i = handles[idx] == r_fds[1] ? 1 : 2;
            DWORD  r;
            if (!ReadFile(r_fds[i], bufs[i] + lens[i],
...
static inline int RK_test_end_parent_loop(RK_test_entry* t, RK_TestHdr hdr,
                                          FILE** w_files, int status,
                                          bool reading_res, char** bufs,
                                          size_t* lens, char* fmtbuf, char* s) {
    ...
    if (act.kind != RK__NORMAL) { /* Others are handled by res already */
        ...
        fwrite(fmtbuf, 1, s - fmtbuf, w_files[0]); // todo
    }
nomore:
    ...
    switch (test_state) {
    case P: fprintf(w_files[0], RK_TFMT_TEST_PASS(t)); break;         // NOLINT
    case F: fprintf(w_files[0], RK_TFMT_TEST_FAIL(t)); break;         // NOLINT
    case C: fprintf(w_files[0], RK_TFMT_TEST_UC(t, act.code)); break; // NOLINT
    case E: fprintf(w_files[0], RK_TFMT_TEST_UE(t, act.code)); break; // NOLINT
    }
    for (int i = 0; i < 3; ++i) {
        if (t->attrs._[i].path) { fclose(w_files[i]); } // todo
        free(bufs[i]);
    }
    free(fmtbuf);
