    ...
    RKT_CONSTRUCTOR(RK_test_suite_register_##__LINE__) {                       \
        ...
        for (s = RKT_glob.suites; s; s = s->next) {                            \
            if (!strcmp(s->name, SUITENAME)) {                                 \
                s->attrs = (RKT_CustomAttrs){0, __VA_ARGS__};                  \
                return;                                                        \
            }                                                                  \
        }                                                                      \
        static RKT_Suite suite;                                                \
        suite.name  = SUITENAME,                                               \
        suite.attrs = (RKT_CustomAttrs){0, __VA_ARGS__};                       \
        if (!RKT_glob.suites) {                                                \
...
#define RK_RUN_TESTS(suites, nsuites, ...)                                     \
    do {                                                                       \
        ...
        RKT_run_all_tests(suites, nsuites, (RKT_CustomAttrs){__VA_ARGS__});    \
                                                                               \
    } while (0)
...
#define rk_assert_crash(signal, ...)                                           \
    do {                                                                       \
        ...
        __VA_ARGS__;                                                           \
        {                                                                      \
            RKT_AssertResPkg RK___PGG = {.res = 1};                            \
            RKT_write_full(RK_OUT, &RK___PGG, sizeof(RK___PGG));               \
            exit(0);                                                           \
        }                                                                      \
    } while (0)
...
static inline size_t RKT_print_assertres(const RKT_TestEntry* restrict e,
    ...
    case RKT_ASSERTRES_PASS:
        if (RKT_resolve_verbosity(e, RKT_PIPE_META) == RK_TEST_LOG_ALWAYS) {
            s += sprintf(s, RK_TFMT_STUB(RK_TFMT_P, e->file, hdr)), *s++ = '\n';
        }
        break;
    case RKT_ASSERTRES_EXIT:
        s += sprintf(s, RK_TFMT_STUB(RK_TFMT_E, e->file, hdr)), *s++ = '\n';
        break;
    case RKT_ASSERTRES_CRASH:
        s += sprintf(s, RK_TFMT_STUB(RK_TFMT_C, e->file, hdr)), *s++ = '\n';
        break;
    case RKT_ASSERTRES_TIMEOUT:
        if (hdr.pos == 0) {
            s += sprintf(s, RK_INRED("[T] %s: TIMEOUT\n"), e->file);
        } else {
            s += sprintf(s, RK_TFMT_STUB(RK_TFMT_T, e->file, hdr)), *s++ = '\n';
        }
        break;
    default:
        ...
        s += sprintf(s, RK_TFMT_STUB(RK_TFMT_F, e->file, hdr)), *s++ = ' ';
        switch (hdr.F) {
        case RKTF_TRUE:
            s += RK_catlit(s, "Evaluated to " RK_INRED("'false'") ".\n");
            break;
        case RKTF_FALSE:
            s += RK_catlit(s, "Evaluted to " RK_INRED("'true'") ".\n");
            break;
        case RKTF_NULL:
            s += sprintf(s, "Evaluated to " RK_INRED("'%s'") ".\n", args[0]);
            break;
        case RKTF_NNULL:
            s += sprintf(s, "Evaluated to " RK_INRED("'%s'") ".\n", args[1]);
            break;
        ...
        case RKTF_MEMZERO:
            s += sprintf(
                s, "%s (%s bytes long) nonzero at byte " RK_INRED("%zu") ".\n",
                args[0], args[1], res.res - 1);
            break;
        case RKTF_MEMNZERO:
            s += sprintf(
                s, "%s (%s bytes long) zero at byte " RK_INRED("%zu") ".\n",
                args[0], args[1], res.res - 1);
            break;
        case RKTF_STREQ:
            s += sprintf(
                s,
                "strings %s, %s " RK_INRED("not equal") " at position %zu.\n",
                args[0], args[1], res.res - 1);
            break;
        case RKTF_STRNEQ:
            s += sprintf(s, "strings " RK_INRED("equal") ".\n");
            break;
        case RKTF_CRASH:
            if (res.res == 1) {
                s += sprintf(s,
                             " - Expected: termination via signal %d, got: no "
                             "termination\n",
                             (int)(hdr.code));
            } else {
                s += sprintf(s,
        ...
        case RKTF_EXIT:
            if (res.res == 1) {
                s += sprintf(s,
                             " - Expected: exit with code %d, got: no exit\n",
                             (int)(hdr.code));
            } else {
                s += sprintf(s,
...
static inline RKT_TestEndType RKT_print_testres(const RKT_TestEntry* restrict e,
    ...
    if (t.capt[RKT_PIPE_STDOUT].dat) {
        ...
        if ((v != RK_TEST_LOG_NEVER)
            && ((v == RK_TEST_LOG_ALWAYS) || t.ended != RKT_PASSED)) {
...
static inline RKT_TestResult RKT_parent_loop(rk_pid pid, rk_fd* pipes,
    ...
    if (reading_res) {
        ...
        if (WIFSIGNALED(status)) {
            if (r.results[r.count - 1].hdr.F == RKTF_CRASH) {
                ...
                res.res = RKT_ASSERTRES_CRASH, r.term_code = WTERMSIG(status);
            }
        } else {
            if (r.results[r.count - 1].hdr.F == RKTF_EXIT) {
                ...
                res.res = RKT_ASSERTRES_EXIT, r.term_code = WEXITSTATUS(status);
            }
        }
        if (res.res) { ++r.fails; }
        r.results[r.count - 1].res = res;
    ...
    if (r.count) {
        size_t last = r.results[r.count - 1].res.res;
        r.ended
            = (status && !reading_res)
                ? RKT_TESTERROR
                : ((last == RKT_ASSERTRES_EXIT || last == RKT_ASSERTRES_CRASH)
                       ? RKT_CRASHED
                       : (r.fails > 0 ? RKT_FAILED : RKT_PASSED));
    }
    return r;
}
        ...
        if (wait == WAIT_FAILED) {
            ...
            if (reading_res) {
                r.results[r.count ? r.count - 1 : 0].res
                    = (RKT_AssertRes){.res = RKT_ASSERTRES_TIMEOUT};
            } else {
                r.results[r.count++] = (RKT_AssertDat){
                    .hdr = {}, .res = {.res = RKT_ASSERTRES_TIMEOUT}};
            }
            ++r.fails;
            return r;
        }
