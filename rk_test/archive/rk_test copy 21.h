...
FILE* JSON_FILE; // todo remove
FILE* BIN_FILE;  // todo remove
...
static void RKT_json_escape(FILE* out, const char* s) {
    for (; *s; s++) {
        switch (*s) {
...
static void RKT_JSON_TOTAL_BEG(FILE* out) {
    fputs("{\n  \"suites\": [\n", out);
}

static void RKT_JSON_TOTAL_END(FILE* out) { fputs("  ]\n}\n", out); }

static void RKT_JSON_SUITE_BEG(RKT_Suite s, FILE* out) {
    fputs("  {\n", out);
    fputs("    \"suite_name\": \"", out);
    RKT_json_escape(out, s.name);
    fputs("\",\n", out);
    fputs("    \"tests\": [\n", out);
}

static void RKT_JSON_SUITE_END(RKT_Suite s, FILE* out) {
    fputs("    ]\n", out);
    fputs("  }", out);
    if (s.next) { fputs(",", out); }
    fputc('\n', out);
...
static void json_escape(FILE* out, const char* s, char* outbuf) {
    for (; *s; s++) {
        switch (*s) {
        case '\"': fputs("\\\"", out); break;
        case '\\': fputs("\\\\", out); break;
...
#define ESCAPE(str)
static void json_write(FILE* out, const char* str, int indent) {
    for (int i = 0; i < indent; ++i) { fputs("    ", out); }
    json_escape(out, str);
}
static void RKT_JSON_TEST(RKT_TestEntry e, RKT_TestResult r, FILE* out,
                          int indent) {
    char escape_buf[1028];
    json_write(out, "{\n", indent);
    json_write(out, "      \"test_name\": \"", indent);

    // test_name
    fputs("      \"test_name\": \"", out);
    RKT_json_escape(out, e.name.str), fputs("\",\n", out);

    fputs("      \"file\": \"", out);
    RKT_json_escape(out, e.file.str), fputs("\",\n", out);

    fputs("      \"tags\": \"", out);
    RKT_json_escape(out, e.attrs.tags ? e.attrs.tags : ""), fputs("\",\n", out);
    ...
    if (!r.count) {
        ...
        for (size_t i = 0; i < r.count; i++) {
            ...
            fputs("          {\n", out);
            fputs("            \"expr\": \"", out);
            RKT_json_escape(out, ad->hdr.exprstr), fputs("\",\n", out);
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                if (j > 0) { fputs(", ", out); }
                fputs("\"", out);
                RKT_json_escape(out, ad->res.args[j]), fputs("\"", out);
            }
            fputs("]\n", out);
            fputs("          }", out);
...
static inline void RKT_run_suite(const RKT_Suite* s) {
    RKT_JSON_SUITE_BEG(*s, JSON_FILE);
    ...
    RKT_JSON_SUITE_END(*s, JSON_FILE);
}

static inline void RKT_run_all_tests(const char**    suites_strs,
                                     size_t          nsuites_strs,
                                     RKT_CustomAttrs attrs) {
    JSON_FILE = fopen("json_output.json", "w"); // todo remove
    RKT_JSON_TOTAL_BEG(JSON_FILE);
...
static inline RKT_TestEndType RKT_run_test(const RKT_TestEntry* e) {
    ...
    RKT_JSON_TEST(*e, results, JSON_FILE);
...
static inline RKT_TestEndType RKT_print_testres(const RKT_TestEntry* restrict e,
    ...
    for (int i = RKT_PIPE_STDOUT; i <= RKT_PIPE_STDERR; ++i) {
        ...
        if ((v != RK_TEST_LOG_NEVER)
            ...
            fwrite(r.capt[i].str, 1, r.capt[i].len, files[i]);
        }
    }
...
static inline rk_pid RKT_init_test(const RKT_TestEntry* restrict e,
    ...
    fflush(JSON_FILE); // <-- Add this todo
