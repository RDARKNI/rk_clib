...
#define RK_TFMT_STUB(filename, hdr)     "%s:%d %s", filename, hdr.pos, hdr.exprstr
...
typedef struct RKT_EntryRuntime {
    rk_fd  pipes[3];
    FILE*  files[3];
    rk_pid pid;
    struct {
...
structured test messages/results
format afterwards
...
typedef struct RKT_EntryResult {
    struct {
        RK_TestHdr hdr;
        struct {
            size_t res;
...
static inline int  RK_test_parent_loop(RK_test_entry* t);
static inline int  RK_test_end_parent_loop(RK_test_entry* t, RK_TestHdr hdr,
                                           int status, bool reading_res,
                                           char** bufs, size_t* lens, char* buf,
                                           char* s);
...
static inline size_t RK_test_log_failmsg(RK_TestHdr hdr, RK_TestRes res,
    ...
    char*       s = buf;
    const char* args[3];
    for (int i = 0, len; fmtlen > 0; ++i) {
        assert(i < 3);
        args[i] = fmtbuf, len = strlen(args[i]) + 1;
...
static inline RKT_EntryRuntime RKT_test_init_test(RK_test_entry* t);
...
NORETURN static inline void RKT_fatal(RKT_EntryRuntime* t, const char* str) {
    perror(str);
    if (t->pid > 0) {
        kill(t->pid, SIGKILL);
        while (waitpid(t->pid, NULL, 0) == -1) {
            if (errno != EINTR) {
                perror("waitpid");
                break;
            }
...
static inline void RKT_setup_files(RKT_EntryRuntime* t, RK_test_entry* e) {
    for (int i = 0; i < 3; ++i) {
        if (!e->attrs._[i].path) {
            t->files[i] = RK_test_glob.files[i];
        } else if (!(t->files[i] = fopen(e->attrs._[i].path, "w"))) {
            RKT_fatal(t, "fopen");
        }
    }
}
...
static inline int RK_test_run_test(RK_test_entry* t) {
    ...
    RKT_EntryRuntime res
        = RKT_test_init_test(t); // todo make that return a new struct
    return RK_test_parent_loop(&res, t);
}
static inline int RKT_reapchild(RKT_EntryRuntime* t) {
    int status;
    while (waitpid(t->pid, &status, 0) == -1) {
        if (errno != EINTR) { RKT_fatal(t, "waitpid"); }
    };
    t->pid = -1;
    return status;
}
...
int               RKT_buf_init(RKT_buf* buf, size_t len) {}

int               RKT_buf_grow_to(RKT_buf* buf, size_t len) {}
// uberm
static inline int RK_test_parent_loop(RKT_EntryRuntime* t, RK_test_entry* e) {
    enum { INITSIZ = 512 };
    ...
    struct pollfd fds[3]      = {{t->pipes[0], POLLIN, 0},
                                 {t->pipes[1], POLLIN, 0},
                                 {t->pipes[2], POLLIN, 0}};
    ...
    for (int nfds = 3; nfds > 0;) {
        ...
        if (fds[0].revents & POLLIN) {
            if (reading_res == 0) {
                ...
                case RK_IO_RESULT_ERR: RKT_fatal(t, "read meta hdr");
                case RK_IO_RESULT_EOF: fds[0].fd = -1, --nfds; break;
                case RK_IO_RESULT_OK:
                    s += sprintf(s, RK_TFMT_STUB(filename, hdr));
                    ++t->total, reading_res = 1;
                }
            } else {
                switch (rk_read_full_nb(fds[0].fd, &res, sizeof(res))) {
                case RK_IO_RESULT_ERR: RKT_fatal(t, "read meta res");
                case RK_IO_RESULT_EOF: fds[0].fd = -1, --nfds; break;
                case RK_IO_RESULT_OK:
                    if (res.len > caps[0]) {
                        caps[0] *= 2;
        ...
        for (int i = 1; i < 3; ++i) {
            ...
            for (ssize_t r;;) {
                ...
                if (r == -1) {
                    ...
                    RKT_fatal(t, "read out/err res");
                } else if (r == 0) {
                    fds[i].fd = -1, --nfds;
                    break;
                } else if ((lens[i] = lens[i] + r) == caps[i]) {
                    caps[i] *= 2, bufs[i] = (char*)realloc(bufs[i], caps[i]);
                    if (!bufs[i]) { RKT_fatal(t, "realloc"); }
                }
            }
        }
    }
    close(t->pipes[0]), close(t->pipes[1]), close(t->pipes[2]);
...
static inline DWORD RKT_reapchild(RK_test_entry* t) {
    DWORD status;
...
static inline int RK_test_run_test(RK_test_entry* t) {
    ...
    return RK_test_parent_loop(t, r_ends, files);
}
static inline int RK_test_parent_loop(RK_test_entry* t) {
    printf(RK_TFMT_TEST_RUNSTART(t));
    enum { INITSIZ = 512 };
    ...
    if (act.kind != RK__NORMAL) { /* Others are handled by res already */
        if (act.kind != exp.kind) {
            if (act.kind == RK__SIGN) {
                ...
                RK_catlit(fmtbuf, RK_TFMT_C), *s++ = '\n';
            } else if (act.kind == RK__EXIT) {
                ++t->fails, test_state             = E;
                RK_catlit(fmtbuf, RK_TFMT_E), *s++ = '\n';
            }
        } else {
        ...
        fwrite(fmtbuf, 1, s - fmtbuf, t->files[0]); // todo
    }
nomore:
    for (int i = 1; i < 3; ++i) { // write stdout/err
        if (test_state == P ? t->attrs._[i].verbosity == RK_TEST_LOG_ALWAYS
                            : t->attrs._[i].verbosity != RK_TEST_LOG_NEVER) {
            fwrite(bufs[i], 1, lens[i], t->files[i]);
            switch (test_state) {
            case P: fprintf(t->files[0], RK_TFMT_TEST_PASS(t)); break; // NOLINT
            case F: fprintf(t->files[0], RK_TFMT_TEST_FAIL(t)); break; // NOLINT
            case C:
                fprintf(t->files[0], RK_TFMT_TEST_UC(t, act.code));
                break; // NOLINT
            case E:
                fprintf(t->files[0], RK_TFMT_TEST_UE(t, act.code));
                break; // NOLINT
            }
        }
    }
    ...
    for (int i = 0; i < 3; ++i) {
        if (t->attrs._[i].path) { fclose(t->files[i]); } // todo
        free(bufs[i]);
    }
    free(fmtbuf);
...
static inline RKT_EntryRuntime RKT_test_init_test(RK_test_entry* e) {
    RKT_EntryRuntime t = {};
    int              pps[6];
    for (int i = 0; i < 3; ++i) {
        if (pipe(pps + i * 2) < 0) { RKT_fatal(&t, "pipe"); };
    }
    if ((t.pid = fork()) == -1) { RKT_fatal(&t, "fork"); }
    for (int i = 0, end = (t.pid > 0) ^ 1; i < 3; ++i) {
        close(pps[i * 2 + !end]), t.pipes[i] = pps[i * 2 + end];
    }
    if (t.pid == 0) {
        if (dup2(t.pipes[1], STDOUT_FILENO) < 0
            || dup2(t.pipes[2], STDERR_FILENO) < 0) { // redirect pipes
            perror("dup2"), exit(1);
        }
        e->func(t.pipes[0]), _exit(0);
    } else {
        for (int i = 0; i < 3; ++i) { // set pipes nonblocking
            int fd = t.pipes[i], flags = fcntl(fd, F_GETFL, 0);
