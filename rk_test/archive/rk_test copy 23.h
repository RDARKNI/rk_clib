...
///
/*
last functioning version without huge memory buffer
*/
#ifndef RK_TEST_H
#define RK_TEST_H
...
static void RKT_json_escape(const char* s) {
...
static void RKT_print_total_beg_json() {
    if (RKT_glob.output_types[RKT_LOG_JSON]) {
        fputs("{\n  \"suites\": [\n", RKT_glob.output_types[RKT_LOG_JSON]);
    }
}

static void RKT_print_total_end_json() {
    if (RKT_glob.output_types[RKT_LOG_JSON]) {
        fputs("  ]\n}\n", RKT_glob.output_types[RKT_LOG_JSON]);
    }
}

static void RKT_print_suite_beg_json(RKT_Suite s) {
    if (RKT_glob.output_types[RKT_LOG_JSON]) {
        fputs("  {\n", RKT_glob.output_types[RKT_LOG_JSON]);
        fputs("    \"suite_name\": \"", RKT_glob.output_types[RKT_LOG_JSON]);
...
static void RKT_print_suite_end_json(const RKT_Suite* s, size_t total,
                                     size_t failed, size_t crashed,
                                     size_t timed_out, size_t error) {
    if (RKT_glob.output_types[RKT_LOG_JSON]) {
        fputs("    ]\n", RKT_glob.output_types[RKT_LOG_JSON]);
        fputs("  }", RKT_glob.output_types[RKT_LOG_JSON]);
        if (s->next) { fputs(",", RKT_glob.output_types[RKT_LOG_JSON]); }
        fputc('\n', RKT_glob.output_types[RKT_LOG_JSON]);
    }
}

static void RKT_print_test_json(const RKT_TestEntry* e, RKT_TestResult r) {
    FILE* out = RKT_glob.output_types[RKT_LOG_JSON];
    if (out) {
        fputs("    {\n", out);
        fputs("      \"test_name\": \"", out);
        ...
        if (!r.count) {
            ...
            for (size_t i = 0; i < r.count; i++) {
                ...
                fputs("          {\n", out);
                fputs("            \"expr\": \"", out);
                RKT_json_escape(ad->hdr.exprstr), fputs("\",\n", out);
                fprintf(out, "            \"pos\": %d,\n", ad->hdr.pos);
                fprintf(out, "            \"res\": %zu,\n", ad->res.res);
                fputs("            \"args\": [", out);
                for (int j = 0; j < 3; ++j) {
...
static inline void RKT_print_suite_beg_reg(const RKT_Suite* s) {
    if (!RKT_glob.output_types[RKT_LOG_NORMAL]) { return; } // todo
}
static inline void RKT_print_suite_end_reg(const RKT_Suite* s, size_t total,
                                           size_t failed, size_t crashed,
                                           size_t timed_out, size_t error) {
...
static inline void RKT_run_suite(const RKT_Suite* s) {
    RKT_print_suite_beg_json(*s);
    RKT_print_suite_beg_reg(s);
    ...
    RKT_print_suite_end_reg(s, total, failed, crashed, timed_out, error);
    RKT_print_suite_end_json(s, total, failed, crashed, timed_out, error);
}

static inline void RKT_run_all_tests(const char**    suites_strs,
                                     size_t          nsuites_strs,
                                     RKT_CustomAttrs attrs) {
    FILE* default_files[3] = {stdout, NULL, NULL};
    for (int i = 0; i < 3; ++i) {
...
static inline RKT_TestEndType
    RKT_run_test(const RKT_TestEntry* e) { // todo print to meta file
    rk_fd          pipes[3];
    rk_pid         pid = RKT_init_test(e, pipes);
    RKT_TestResult results
        = RKT_parent_loop(pid, pipes, RKT_resolve_timeout(e));

    RKT_print_test_reg(e, results);
    RKT_print_test_json(e, results);
...
#define RKT_VALIDATE(FUNNAME, RESULT)                                          \
    RK_REGISTER_TEST("testmeta", test_##FUNNAME) {                             \
        ...
        rk_assert_eq(RKT_run_test(&RKT_CONCAT(RKT_entry_, FUNNAME)), RESULT);  \
