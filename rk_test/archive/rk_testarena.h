...
static inline RKT_TestEndType RKT_run_test(const RKT_TestEntry* e) {
    ...
    free(results.backing_memory.data);
    return res;
}
...
static inline RKT_TestEndType RKT_print_testres(const RKT_TestEntry* restrict e,
    ...
    printf(RK_INYELLOW("Running %s\n") "---------------------------------------"
                                       "-----------\n",
           e->name.str);
    const RKT_Arena* a      = &r.backing_memory;
    size_t           fmtcap = 1024;
...
static inline RKT_TestResult RKT_parent_loop(rk_pid pid, rk_fd* pipes,
    ...
    RKT_TestResult r       = {.backing_memory = {malloc(2 << 16), 2 << 16, 0},
                              .count          = 0,
                              .fails          = 0};
    if (!r.backing_memory.data) { RKT_fatal(pid, "malloc"); }
    RKT_Arena* a             = &r.backing_memory;
    r.results                = RKT_alloc(a, RKT_AssertDat, rescap);
    r.capt[0].dat            = RKT_alloc(a, char, caps[0]);
    r.capt[1].dat            = RKT_alloc(a, char, caps[1]);
